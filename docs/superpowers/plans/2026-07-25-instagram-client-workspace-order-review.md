...
- [x] **Step 1: Write failing tests** for one client/many commercial episodes, one episode/one intended order, repeat-order detection, retained previous funnels, exact existing-order linking, and no duplicate order after payment confirmation.
- [x] **Step 2: Run tests and verify RED.**
- [x] **Step 3: Add a durable commercial-episode model** that owns its deal, payment review/decision, order attribution, stage timeline, product/price evidence and outcome. A client may have many episodes; opening a repeat purchase starts a new episode without rewriting the completed one.
- [x] **Step 4: Add repeat-order analysis semantics** for explicit repeat intent ("хочу ще", gift/another recipient, reorder) and expose order count, previous success amount/date, current episode and repeat-customer state without guessing from language or profile style.
- [x] **Step 5: Harden existing-order resolution.** Continue exact unique order-number lookup, block cancelled orders, require a structured override for already shipped/completed or payment-incompatible orders, and never mutate the selected Order/OrderItem rows while linking.
- [x] **Step 6: Make automatic materialization episode-aware and idempotent.** Provider-verified payment creates exactly one attributed order only when product/price/delivery truth is complete; incomplete truth creates one manager task. Persist `provider_auto`, `manager_review`, or `linked_existing` creation mode plus display name and durable UID-backed attribution.
- [x] **Step 6a: Keep all price and payment amounts episode-scoped and data-driven.** Never copy a previous client's or previous episode's amount. Resolve negotiated line prices and order total from bounded conversation evidence, resolve provider-paid value from the exact Monobank invoice/projection, and resolve manager-confirmed payment from the exact review. Store agreed total, requested payment amount, and actually paid amount separately; catalog price is only a fallback/reference. Arbitrary valid prepayment amounts must not be silently replaced by a hardcoded `200`.
- [x] **Step 7: Make order-status answers episode-aware.** Resolve exact order number or TTN across deal-linked and attribution-only orders, read its Order/Nova Poshta state, and answer only from stored provider/order truth; ambiguous references to several active orders create one manager clarification task.
- [x] **Step 8: Extend the client/order API and custom admin** with chronological episodes, distinct physical order counts, multiple order numbers, Instagram display name/UID navigation, source/creation mode, shipment update time and immutable historical funnel/evidence snapshots.
- [x] **Step 9: Rework the client workspace around two primary panes.** Keep the client list and conversation as the only always-visible panes; move commercial context, settings, order review and full evidence into an explicit side drawer opened by `Розширено`/gear. Put one compact animated funnel row above the conversation, keep manager/bot/customer message colors legible, and pulse the client/order row only when a manager action is required. Restore a compact potential/readiness strip above the chat using the evidence-bound analysis band, probability and confidence (`cold`, `exploring`, `qualified`, `high_intent`, `checkout`, `lost`, `opted_out`, spam); verified payment/order truth must remain a separate factual badge and never be inferred from the visual score.
- [x] **Step 10: Replace manual order-number-only linking with compact existing-order selection.** Add a staff API that returns only safe candidate summaries (order number/date/client/items/amount/payment/shipment/source), allow one-click selection with exact-number fallback, show mismatch/override state before save, and immediately bind the selected order to the same Instagram client/episode.
- [x] **Step 10a: Keep the full review evidence reachable in every drawer state.** The right drawer uses one viewport-bounded scroll surface; product photos, receipt screenshots, custom-print evidence, decision history and the final actions remain reachable after candidate search, link-existing and create-new transitions at desktop and 390/320 px. Sticky actions do not cover the last evidence block.
- [ ] **Step 11: Run focused repeat-order/payment/order/shipment/UI suites and commit** `feat: model repeat instagram commercial episodes`. Local verification is complete (555 acceptance tests, 2 expected skips), including release-window reconciliation regressions; independent review is `APPROVED`. The release commit and production deployment remain the next gate.
...
- The client screen has two primary panes; context/settings are a contextual drawer, not a permanent third pane.
- The conversation starts with one compact, single-row funnel; action-required payment/order states are visually prominent but not duplicated in multiple cards.
- Client potential/readiness is visible above the transcript with probability/confidence and an explainable label, while payment/order facts remain source-qualified and visually separate.
- Existing-order linking is a compact searchable selection with product/date/status evidence, not a blind number input; the exact number remains a fallback for audited edge cases.
