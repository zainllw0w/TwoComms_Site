...
| Поле | Значение |
|---|---|
| Текущая фаза | **W0 завершена (диагностика). Код НЕ менялся. Следующая — W1** |
| Дата старта / обновления | 2026-08-01 |
| Baseline commit | `2f75f9d9` (main, синхронен с origin/main и с продом) |
| **Изменения в коде** | **НЕТ.** Только файлы в `docs/instagram_bot_audit/` |
...
2. Получить 4 решения заказчика (промокод 10%, checkout-домен,
   `IgLifecycleEvent`, приоритет W5 против W7). W1 от них не зависит.
3. **Начать W1** (два P0 по безопасности данных) — от W0 не зависит,
   можно стартовать сразу.
4. Перед W2 — повторная проверка доли 403 (см. выше), затем строго
   **IMP-008 → IMP-009** (guard'ы до восстановления cron, иначе фикс
   инфраструктуры сам создаст инцидент с рассылкой).
