...
| Поле | Значение |
|---|---|
| Текущая фаза | **W4 и W4C закрыты. W4B — ядро закрыто, остаток открыт** |
| Дата старта / обновления | 2026-08-02 |
| Baseline аудита | `2f75f9d9` — ⚠️ **устарел, см. «Расхождение baseline» ниже** |
| База внедрения | `c696ee9e2615fa1f87bdb2d3da554446f8177ff1` (origin/main), ветка `codex/ig-bot-w4-completion` |
| **Изменения в коде** | **ЕСТЬ (W4 + W4C).** IMP-020–024, IMP-054, IMP-063–072; миграция `0128` |
...
## W4 реализована локально — публикация ещё не подтверждена (2026-08-02)

- IMP-020/021 из прежних W4/W4B изменений подтверждены в `origin/main`;
  текущий срез завершает IMP-022/023/024 и сверяет IMP-054.
...
- Финальный commit, push, deploy и проверка серверной MySQL будут записаны
...
Не сделано: **IMP-022, IMP-023, IMP-024, IMP-051, IMP-053, IMP-054, IMP-055,
IMP-056, IMP-057, IMP-058.**
...
- **IMP-058 к тому же заблокирован** зависимостью на IMP-032 (полнота событий
  стадии, волна W6) — без неё воронка переходов будет с дырами, и статистика
  падений начнёт измерять свои же пробелы.
- **IMP-024 (промокод 10% за UGC)** требует решения заказчика: убрать обещание /
  ручная выдача / автоотслеживание. Без решения любая реализация — угадывание.
- **IMP-022, IMP-023 (тексты денежных сообщений и follow-up)** зависят от
  липкости языка (IMP-029, волна W5). Разведка измерила масштаб: у **99 из 168**
  клиентов с 2+ определениями язык меняется хотя бы раз, **229 переключений**
  всего, у 93 клиентов в диалоге есть и ru, и uk. Локализовать тексты до
  стабилизации языка — значит переводить их на язык, который выберется случайно.
...
**Что делать следующим агентом в первую очередь:** IMP-054 (quiet hours —
две несогласованные конфигурации тишины, дёшево и снижает риск), затем
IMP-029 (липкость языка — самый сильно подтверждённый данными пункт всей W5),
затем IMP-053.
