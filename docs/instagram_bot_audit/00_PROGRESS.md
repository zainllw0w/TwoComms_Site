...
| `03_FINDINGS_REGISTER.md` | 163 уникальных `F-*` и post-implementation evidence, включая production SQL/API |
...
7. **Через сутки после деплоя** пересчитать долю 403 на `/bot/webhook/` по
   ...
   ошиблась W0.
8. Остаток W4B: IMP-056, IMP-057, IMP-058 и IMP-089.
   IMP-051/053/055 закрыты и задеплоены; следующий по порядку — событийный
   слой и двухфазный claim IMP-056.
...
IMP-051/053/055 позднее закрыты на production; IMP-056–058 остаются открыты.
...
IMP-051. IMP-053/051/055 выполнены; текущий порядок продолжения — IMP-056 ->
...
**Остаток W4B не спрятан внутри этого закрытия:** IMP-056
