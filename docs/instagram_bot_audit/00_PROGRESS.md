...
| Поле | Значение |
|---|---|
| Текущая фаза | **IMP-080 и W7 задеплоены; идёт сверка остатка W4B/W5/W8 и незавершённых веток** |
| Дата старта / обновления | 2026-08-03 |
| Baseline аудита | `2f75f9d9` — ⚠️ **устарел, см. «Расхождение baseline» ниже** |
| База внедрения | `1380db8e6d3fe9d0876320f58f6e36875ad13dca` (`origin/main` = production), ветка `codex/ig-bot-variant-pricing` |
| **Изменения в коде** | **Задеплоены W0–W7 (с явно открытым остатком W4B/W5), IMP-077 partial и IMP-080.** Последний кодовый срез: IMP-035–040 |
...
| Находок всего | **114** (+ F-CAT-003, F-DATA-016, F-UX-016) |
| Из них P0 | **23** |
| Улучшений (не багов) | **48** + дизайн добивки воронки |
| Задач чек-листа закрыто | **120 / 120** (домены A–L) |
| Задач в плане внедрения | **67** в 11 волнах; W9 добавлена из незавершённой ветки assisted checkout |

## Статусы и ветки — источник истины

- `[x]` ставится только когда код находится в `origin/main`, задеплоен и имеет
  свежую проверку. Сам факт коммита в feature-ветке не закрывает задачу.
- `[ ] РЕАЛИЗОВАНО В ВЕТКЕ` означает сохранённую работу, которую ещё нужно
  перенести на актуальный `main`, перепроверить и задеплоить.
- `[ ] ЧАСТИЧНО` означает, что часть требований опубликована, но перечисленный
  остаток всё ещё обязателен.

Сверка `git log --cherry-pick --right-only origin/main...<branch>` от 2026-08-03:

| Ветка / worktree | Patch-unique относительно `origin/main` | Решение |
|---|---:|---|
| `ig-bot-w1-data-safety`, `ig-bot-w2-throughput`, `ig-bot-w3-post-sale` | 0 | перенесено в `main`; не переисполнять |
| `ig-bot-w4-completion`, `ig-order-fulfillment-links` | 0 | коммиты перенесены; грязный W4 pagination-прототип замещён W7 |
| `ig-refresh-dedup`, `instagram-login-runtime`, `ig-sales-bot-upgrade` | 0 | перенесено или patch-equivalent `main` |
| `ig-crm-master-audit` | 0 committed | грязный старый Meta transport замещён текущим `provider_transport`; production работает на `instagram_login`, wholesale merge запрещён |
| `instagram-assisted-checkout` | **7 commits** | W9: `4fd85d60`, `5ec4885f`, `61ad2cb8`, `a8ccfa63`, `468fe2ba`, `e9d982df`, `dc9889c3`; переносить по одному с новой проверкой |

Локальный checkout `main` намеренно не переключается: он грязный и отстаёт.
Интеграционная истина — `origin/main` и server SHA, а не указатель локальной
ветки в чужом worktree.
...
| `03_FINDINGS_REGISTER.md` | 114 находок и post-implementation evidence, включая production SQL/API |
...
| `07_IMPLEMENTATION_PLAN.md` | 59 задач, 10 волн (W0–W8 + W4B), фикс+улучшение одним срезом |
...
## W4 опубликована; W4B остаётся частичной (2026-08-02, сверено 2026-08-03)
...
- W4B не закрыта: IMP-051, IMP-053, IMP-055, IMP-056, IMP-057 и IMP-058 открыты.
- Production уже работает на последующем SHA `1380db8e`, который включает W4;
  серверная БД подтверждена как MySQL.
...
7. **Через сутки после деплоя** пересчитать долю 403 на `/bot/webhook/` по
   ...
   ошиблась W0.
8. Остаток W4B: IMP-051, IMP-053, IMP-055, IMP-056, IMP-057, IMP-058.
   Первым — IMP-053 (policy-таблица каскадов), потому что после W4C именно
   follow-up остался единственным местом с заготовленными текстами.
9. Решения заказчика: checkout-домен, `IgLifecycleEvent`, приоритет W5 против
   W7. Промокод 10% решён в W4 (вариант B — ручная выдача).
10. **Занести в W8:** F-DEBT-006 (предсуществующие красные тесты),
...
Не сделано: **IMP-051, IMP-053, IMP-055, IMP-056, IMP-057, IMP-058.**
...
- **Зависимость IMP-058 на IMP-032 уже снята в W6:** FSM и журнал переходов
  опубликованы. Сам IMP-058 остаётся отдельной большой подсистемой статистики,
  а не автоматически закрытой задачей.
- **IMP-051 (перенос `poll_pending_deals` в тикер демона)** снимает класс отказа
  ...
  главная причина, по которой IMP-051 стоял рядом с IMP-050, уже снята.

**Что делать следующим агентом в первую очередь:** IMP-053. IMP-054 и IMP-029,
которые прежняя запись ставила перед ним, уже закрыты и повторно проверены.
