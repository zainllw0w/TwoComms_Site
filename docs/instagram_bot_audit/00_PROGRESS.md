...
| Поле | Значение |
|---|---|
| Текущая фаза | **W0, W1, W2 завершены. Следующая — W3 (истина о покупателе, IMP-013)** |
| Дата старта / обновления | 2026-08-01 |
...
**Радиус восстановления cron замерен заранее** (read-only): 0 сделок-кандидатов,
1 эпизод — `IgClient#59` на паузе и вне окна ответа. Ни одного сообщения
клиенту при включении не уходит.
...
4. **W3 — истина о покупателе.** Ключевая задача **IMP-013**, читать **DR-001**:
   расширять `verified_payment_q` глобально нельзя, иначе система начнёт
   создавать заказы по скриншоту чека. Критерий приёмки волны проверяемый:
   карточка `IgClient#59` показывает «оплачено · обмін XL у дорозі»
   вместо «cold · Підтримка / скарга · 0%».
5. Получить 4 решения заказчика (промокод 10%, checkout-домен,
   `IgLifecycleEvent`, приоритет W5 против W7). W3 от них не зависит.
6. Повторить проверку доли 403 после нескольких активных дней трафика —
