...
| Поле | Значение |
|---|---|
| Текущая фаза | **W0 и W1 завершены. Следующая — W2 (строго IMP-008 → IMP-009)** |
...
1. ~~Закрыть 4 открытых вопроса W0~~ — **сделано**.
2. ~~W1: два P0 по безопасности данных~~ — **сделано**.
3. Получить 4 решения заказчика (промокод 10%, checkout-домен,
   `IgLifecycleEvent`, приоритет W5 против W7).
4. Перед W2 — повторная проверка доли 403 на `/bot/webhook/` (см. остаток W0),
   затем строго **IMP-008 → IMP-009** (guard'ы до восстановления cron,
   иначе фикс инфраструктуры сам создаст инцидент с рассылкой).
5. Дёшево и по пути: F-DEBT-005 (одна строка) и инвариант-тест из IMP-004.
