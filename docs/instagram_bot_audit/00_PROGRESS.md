...
| Поле | Значение |
|---|---|
| Текущая фаза | **W4B event/claim слой IMP-056 и lifecycle возражений IMP-057 закрыты; активный остаток: IMP-058, IMP-089, затем W8/W9/W10** |
| Дата старта / обновления | 2026-08-03 |
| Исходный baseline аудита | `2f75f9d9` — исторический, больше не использовать для новых веток |
| База внедрения | `6b86e103` подтверждён в `origin/main` и на production; включает reliability-срез поверх fulfillment IMP-055, durable claims IMP-056 и lifecycle возражений IMP-057 |
| **Статус 97 IMP-задач** | **69 закрыты, 26 открыты, 2 частично закрыты (`IMP-043`, `IMP-077`)** |
...
| Реестр находок | **169 уникальных `F-*` идентификаторов**; добавлены F-CORE-018, F-AI-017 и F-CAT-004, повторные записи сохраняют историю проверки и закрытия |
| Улучшения / решения | **48 `IMPR-*` / 9 `DR-*`** |
| Задач чек-листа закрыто | **120 / 120** (домены A–L) |
| Задач в плане внедрения | **97** в W0–W10, включая W4B/W4C/W4D и IMP-062…097 |
...
| `03_FINDINGS_REGISTER.md` | 169 уникальных `F-*` и post-implementation evidence, включая production SQL/API |
...
| `07_IMPLEMENTATION_PLAN.md` | канонический статус всех 97 IMP-задач: 69 закрыты / 26 открыты / 2 partial |
...
- W4B не закрыта: IMP-058 и найденная при payment-аудите IMP-089 открыты;
...
7. **Через сутки после деплоя** пересчитать долю 403 на `/bot/webhook/` по
   ...
   ошиблась W0.
8. Остаток W4B: IMP-058 и IMP-089.
...
IMP-051/053/055/056/057 позднее закрыты на production; открыта только IMP-058.
...
IMP-051. IMP-053/051/055/056/057 выполнены; текущий следующий шаг — IMP-058.
...
backstop IMP-089. Позднее IMP-056/057 закрыты; текущий остаток — IMP-058/089.
