...
| Поле | Значение |
|---|---|
| Текущая фаза | **Ветки и WIP-документы восстановлены в `main`; активный остаток: W4B, один пункт W5, W8 и branch-only W9** |
| Дата старта / обновления | 2026-08-03 |
| Исходный baseline аудита | `2f75f9d9` — исторический, больше не использовать для новых веток |
| База внедрения | `cd070cba` подтверждён в `origin/main` и на production; включает policy-каскады IMP-053 поверх W0-W8 |
| **Статус 88 IMP-задач** | **64 закрыты, 22 открыты, 2 частично закрыты (`IMP-043`, `IMP-077`)** |
...
| Реестр находок | **160 уникальных `F-*` идентификаторов**; повторные записи в реестре сохраняют историю проверки и закрытия |
| Улучшения / решения | **48 `IMPR-*` / 9 `DR-*`** |
| Задач чек-листа закрыто | **120 / 120** (домены A–L) |
| Задач в плане внедрения | **88** в W0–W9, включая W4B/W4C/W4D и добавленные IMP-062…088 |
...
| `03_FINDINGS_REGISTER.md` | 160 уникальных `F-*` и post-implementation evidence, включая production SQL/API |
...
| `07_IMPLEMENTATION_PLAN.md` | канонический статус всех 88 IMP-задач: 64 закрыты / 22 открыты / 2 partial |
...
| Статус | Задачи |
|---|---|
| Открыто, W4B | `IMP-051`, `IMP-055`–`IMP-058` |
| Открыто, W5 | `IMP-028` |
| Открыто, W8 | `IMP-041`, `IMP-042`, `IMP-044`–`IMP-046`, `IMP-059`–`IMP-061` |
...
- W4B не закрыта: IMP-051, IMP-055, IMP-056, IMP-057 и IMP-058 открыты;
  IMP-053 закрыта отдельным production-срезом `cd070cba`.
- Production уже работает на последующем SHA `cd070cba`, который включает W4;
...
7. **Через сутки после деплоя** пересчитать долю 403 на `/bot/webhook/` по
   ...
   ошиблась W0.
8. Остаток W4B: IMP-051, IMP-055, IMP-056, IMP-057, IMP-058.
   IMP-053 (policy-таблица каскадов) закрыта и задеплоена на `cd070cba`;
   следующий по порядку — IMP-051, затем IMP-055 и событийный слой IMP-056.
...
IMP-053 позднее закрыта на production, остальные пять остаются открыты.
...
**Историческая рекомендация этого checkpoint была:** начать с IMP-053. Она уже
выполнена; текущий порядок продолжения — IMP-051 -> IMP-055 -> IMP-056 ->
IMP-057 -> IMP-058.
...
**Остаток W4B не спрятан внутри этого закрытия:** IMP-051 (payment polling в
daemon), IMP-055 (настоящий customer-facing `Kind.FULFILLMENT`), IMP-056
(event trigger + двухфазный claim), IMP-057 (жизненный цикл возражений) и
IMP-058 (cohort/drop-off статистика) остаются открыты. Реальных сообщений
клиентам при production-проверке IMP-053 не отправлялось.
