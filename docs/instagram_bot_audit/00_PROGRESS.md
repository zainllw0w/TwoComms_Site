...
| Поле | Значение |
|---|---|
| Текущая фаза | **W0–W2 закрыты. W3 в работе: IMP-013 ✅, IMP-014 ✅, дальше IMP-015** |
...
**Что осталось в W3:** IMP-015 (обмен ≠ жалоба), IMP-016 (не продавать при
открытом сервисном кейсе), IMP-017 (ранжирование паттернов), IMP-018
(терминальные факты при выборе снапшота), IMP-019 (честная подпись метрики).
Плюс новая задача **IMP-062** — привязка ТТН обмена к тому же заказу
(прямой запрос заказчика, дизайн в разделе W3 реестра находок).

## Следующий шаг

1. ~~W0: 4 открытых вопроса~~ — **сделано**.
2. ~~W1: два P0 по безопасности данных~~ — **сделано**.
3. ~~W2: проходимость контура (IMP-008 → IMP-009 → 010 → 011 → 012)~~ — **сделано**.
4. ~~IMP-013, IMP-014~~ — **сделано и задеплоено** (`3d7f8a46`).
5. **IMP-015** — обмен и возврат перестают быть жалобой. Это вторая половина
   критерия приёмки волны: `interaction_type` клиента #59 должен стать
   `exchange_request`, а не `support_complaint`.
6. Получить 4 решения заказчика (промокод 10%, checkout-домен,
   `IgLifecycleEvent`, приоритет W5 против W7). W3 от них не зависит.
