...
| Поле | Значение |
|---|---|
| Текущая фаза | **IMP-051 опубликована; активный остаток: W4B, один пункт W5, W8, branch-only W9 и восстановленный backlog W10** |
| Дата старта / обновления | 2026-08-03 |
| Исходный baseline аудита | `2f75f9d9` — исторический, больше не использовать для новых веток |
| База внедрения | `2a89d860` подтверждён в `origin/main` и на production; включает daemon payment backstop IMP-051 поверх policy-каскадов IMP-053 |
| **Статус 93 IMP-задач** | **65 закрыты, 26 открыты, 2 частично закрыты (`IMP-043`, `IMP-077`)** |
...
| Реестр находок | **161 уникальный `F-*` идентификатор**; добавлена F-PAY-014, повторные записи сохраняют историю проверки и закрытия |
| Улучшения / решения | **48 `IMPR-*` / 9 `DR-*`** |
| Задач чек-листа закрыто | **120 / 120** (домены A–L) |
| Задач в плане внедрения | **93** в W0–W10, включая W4B/W4C/W4D и добавленные IMP-062…093 |
...
| `03_FINDINGS_REGISTER.md` | 161 уникальный `F-*` и post-implementation evidence, включая production SQL/API |
...
| `07_IMPLEMENTATION_PLAN.md` | канонический статус всех 93 IMP-задач: 65 закрыты / 26 открыты / 2 partial |
...
| Статус | Задачи |
|---|---|
| Открыто, W4B | `IMP-055`–`IMP-058`, `IMP-089` |
| Открыто, W5 | `IMP-028` |
...
- W4B не закрыта: IMP-055–058 и найденная при payment-аудите IMP-089 открыты;
  IMP-051/053 закрыты отдельными production-срезами `2a89d860`/`cd070cba`.
- Production уже работает на последующем SHA `2a89d860`, который включает W4;
...
7. **Через сутки после деплоя** пересчитать долю 403 на `/bot/webhook/` по
   ...
   ошиблась W0.
8. Остаток W4B: IMP-055, IMP-056, IMP-057, IMP-058 и IMP-089.
   IMP-051/053 закрыты и задеплоены; следующий по порядку — настоящий
   fulfillment IMP-055, затем событийный слой IMP-056.
...
IMP-051/053 позднее закрыты на production; IMP-055–058 остаются открыты.
...
IMP-051. Обе выполнены; текущий порядок продолжения — IMP-055 -> IMP-056 ->
IMP-057 -> IMP-058. IMP-089 остаётся отдельным payment-recovery остатком.
...
**Остаток W4B не спрятан внутри этого закрытия:** IMP-055 (настоящий
customer-facing `Kind.FULFILLMENT`), IMP-056
...
backstop IMP-089 остаются открыты. Реальных сообщений
...
Все 48 `IMPR-*` получили явный `DONE/PARTIAL/OPEN/REFRAMED` и ссылку на
каноническую задачу в `05_IMPROVEMENTS_REGISTER.md`. Добавлены IMP-090–093 для
