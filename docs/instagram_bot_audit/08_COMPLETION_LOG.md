...
| Implement2 W2.1 authoritative order lifecycle and delivery truth | `51db3058`, `8d8c5d05` | Released to production on 2026-08-14 after exact-main disposable MariaDB lifecycle proof (`29/29`, CI `31813850156`/`31814617538`) and explicit application of migration `management.0156`. Production SHA is exact; MariaDB schema proof is `varchar(255)` plus `LONGTEXT`/`JSON_VALID`; `manage.py check`, management migration drift, daemon/provider/queue health and both HTTP health endpoints are green. Canonical lifecycle messages/send markers/receipts stayed at zero; legacy events `5` and the historical delivered fact `1` were unchanged. Full `IMP-087`/`IMP-094` and the separate storefront drift remain open. |
