...
| Implement2 W2.4 / `IMP-044` partial | `a6dd2882` | Generic/background Gemini attempts now use an atomic project-key lease directly around provider I/O, skip busy keys, release the exact token in `finally`, and clip provider timeouts/backoff/new attempts to the remaining overall deadline and 70-second lease budget. Manual-key and live-chat behavior are unchanged. Focused gate `3/3`; production exact-SHA, clean tracked tree and healthy bot/queue proof are recorded in `09`. Full `IMP-044` remains PARTIAL for slow-drip hard cancellation, typed telemetry/heartbeat, derived health, allowlist/UI/jitter, migration `0169` and disposable MariaDB races. |
