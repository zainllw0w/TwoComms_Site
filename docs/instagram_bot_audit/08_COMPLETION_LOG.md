...
| Implement2 W2.4 / `IMP-044` partial | `a6dd2882`, `c1dc6e278` | Generic/background Gemini attempts use an atomic project-key lease directly around provider I/O, skip busy keys, release the exact token in `finally`, and clip provider timeouts/backoff/new attempts to the remaining overall deadline and 70-second lease budget. The follow-up derives current key health from env/cooldown/project lease and makes the checker fail closed while preserving legacy `available`; manual-key and live-chat behavior are unchanged. Focused gate `66/66` plus check/compile/diff passed; production exact-SHA, clean tracked tree, six-row key-health smoke and healthy bot/queue proof are recorded in `09`. Full `IMP-044` remains PARTIAL for slow-drip hard cancellation, typed telemetry/heartbeat, allowlist/UI options/jitter, migration `0169` and disposable MariaDB races. |
| Implement2 W2.4 Gemini API Checker | `7372f0b6b`, follow-ups `8bfb6c5ea`/`7fde498fb`, API proof `7b289ab05`, docs proof `c6ee5697b` | Admin-only `API` tab with six key capsules, live/ready/degraded/offline/stale states, separate 3.7/3.6 24-hour rails, passive refresh/countdown and explicit manual probe. Hourly token-free metadata checker is 3.7-first, conditionally checks 3.6, uses one deadline/lock and one managed cron owner. Original release gate `70/70`, refreshed gate `53/53`, cron gate `18/18`; current canonical main/production reconciliation is `debcef315` (with `7b289ab05` as the API-proof ancestor). `IMP-044` remains PARTIAL. |
| Implement2 W2.4 API Checker alignment follow-up | `c64dc224b` | All six hourly alias probes start concurrently while preserving per-alias `3.7 -> conditional 3.6`; late evidence is rejected, joined workers cannot outlive ownership, and coordinator ledger writes are atomic. Fixed 320 px desktop evidence track plus responsive local rail scrolling removes 3.6 drift. Fresh gate `121/121`, cron `18/18`, browser matrix 1920/1280/640/375 with zero page overflow. Production exact SHA `c64dc224b`; MariaDB snapshot: six keys, two models per key, attempts `89 -> 89`, key states `6 -> 6`; `IMP-044` remains PARTIAL. |
