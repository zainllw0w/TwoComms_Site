...
| Implement2 W2.4 Gemini API Checker | `7372f0b6b`, docs proof `c6ee5697b` | Admin-only `API` tab with six key capsules, live/ready/degraded/offline/stale states, separate 3.7/3.6 24-hour rails, passive refresh/countdown and explicit manual probe. Hourly token-free metadata checker is 3.7-first, conditionally checks 3.6, uses one deadline/lock and one managed cron owner. Focused checker/API/metadata gate `70/70`, cron gate `18/18`; current canonical main/production reconciliation is `6370b1823`. `IMP-044` remains PARTIAL. |
