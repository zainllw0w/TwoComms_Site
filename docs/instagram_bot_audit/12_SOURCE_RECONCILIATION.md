...
| `codex/ig-w22-t41-sanitizer-followup` | disposable MariaDB lifecycle and checkout-concurrency runner slices `d054edf0e`…`8f4459f68`, including strict failure-evidence allowlists | BRANCH-ONLY. Exact-SHA CI `31761170448` proves the narrow lifecycle/checkout gate on MariaDB 11.4.12 with generated schemas and verified cleanup; no production completion may be inferred before current `main`, approved SSH `git pull`, and read-only runtime proof. The remaining append-only `TransactionTestCase` classes need individual MariaDB treatment; do not globally weaken triggers. |
