...
| `codex/ig-implement2-w13`, `codex/ig-implement2-w14`, `codex/ig-implement2-w16`, `codex/ig-implement2-w17` | historical W1.2/W1.4/W1.5/W1.6 implementation-plan checkpoints | IN MAIN / production through the later rebased commits (`W1.6` at `130cd920`; W1.7 at `214ae4b9`/`b9bab236`). Their old-base tips are documentation sources only; do not cherry-pick wholesale. |
| `codex/ig-w22-t41` | disposable MariaDB sanitizer and checkout-concurrency evidence (`1fd53fea1`) | IN MAIN / production through the rebased seven-commit sequence at `9ed640b06c`; the branch tip itself is not an ancestor of current `main`. Its audit files are older snapshots and would delete newer evidence if merged wholesale. |
| `codex/ig-follow-intelligence` (`dba5a424`, `988bb4552`) | historical follow-state/CTA lifecycle implementation on an old base | PATCH-UNIQUE REQUIREMENTS PRESERVED, code already represented more completely in current `main` (`c270704a9` through `0eac60f05`, migrations `0157`–`0166`). Do not cherry-pick either tip: the branch wholesale diff rolls back current runtime and audit files. |
