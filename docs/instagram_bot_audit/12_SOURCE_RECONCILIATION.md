...
| `codex/ig-commerce-durable-state` worktree | modified `services/ig_commerce_state.py`, `services/instagram_bot.py`, `tests_ig_commerce_state.py`; new `services/ig_commerce_replies.py`, `tests_ig_commerce_delivery.py` | WIP `IMP-087.A`, uncommitted/unpushed/undeployed. HEAD equals baseline `19f5ef70`; local branch is 14 ahead / 1 behind its own remote ref, so never overwrite or push blindly. Fresh evidence: 101 tests + check/migration drift/compile/diff. Narrow deterministic informational reply only; general `IMP-087` remains PARTIAL. |
