...
> Состояние карты: сверено с `main`/production code `fbe33a68` на 2026-08-05.
...
| Добивка | `bot_followups.process_due_followups` | `IgFollowUpTask`, immutable event facts, absolute policy timeline, delivery state, lease, provider receipt | event-driven continuation and provider-evidenced delivery FSM current on `fbe33a68` |
