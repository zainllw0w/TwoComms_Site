...
То есть при включении cron **ни одного сообщения клиенту не уйдёт**.
Единственный кандидат — тот самый клиент #59 из жалобы заказчика, он на паузе,
и новый guard отправит задачу менеджеру с причиной `send_blocked:bot_paused`
вместо сообщения.
