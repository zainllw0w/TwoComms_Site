...
| `IgFollowUpTask` | scheduled touch/manager task | immutable event key/payload/time, absolute policy timeline, delivery state + lease + provider receipt | event-driven continuation and provider-evidenced FSM included in current `fbe33a68` runtime |
...
| follow-up | policy step, immutable event fact, claim, provider receipt, ambiguous delivery, manager review, cancelled | event continuation and delivery FSM included in current `fbe33a68` runtime |
...
Follow-up delivery truth is included in current `fbe33a68` (feature slice first published
through `434428ad`): explicit
