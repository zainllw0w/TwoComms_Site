...
| [ ] | IMP-106 — CORE SHIPPED; EXTERNAL/POLICY GATES OPEN | Follow-state, demand-driven CTA/lifecycle guards, manager UX and migrations `0157`–`0166` are in current `main`/production. The consented Graph capability probe, coupon calibration, privacy retention and biometric-policy decisions remain open. | Without a consented target and correctly matched app/token, state remains `unknown` and CTA is suppressed; no live customer probe merely to close the item. |
...
- [ ] T41 — SQLite suite GREEN и narrow checkout-concurrency gate GREEN on exact-main disposable MariaDB CI (`31762702125` / `9ed640b06c`, strict failure-evidence allowlist, exact schema evidence, `cleanup=verified`); the approved SSH pull and production running/zero-queue proof are recorded, but the full management parity matrix and deterministic current command remain open.
