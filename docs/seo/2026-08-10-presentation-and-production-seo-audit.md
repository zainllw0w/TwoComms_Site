...
Статус: доказательный read-only аудит завершен 2026-08-11; исправления в этой задаче не выполнялись
...
Исключение: рабочий поток custom print не предлагается перерабатывать или ломать. Он проверяется только на предмет границ индексации, ссылок и возможного влияния на остальные страницы.
...
### 2.4. Нормативная база Google, полученная через Context7
...
#### FIND-006 — 73 индексируемые страницы `цвет x посадка` продвигаются sitemap, но часть из них наследует meta только от цвета

- **Вердикт:** подтверждено в production и коде; это не ошибка самого формата variant URL, а системная ошибка реализации уникальности и выбора index target.
- **Приоритет:** P1 для архитектурного решения и устранения дублей; не удалять URL массово без карты спроса и редиректов.
- **Масштаб:** live `sitemap-product-variants.xml` содержит 210 URL: 78 color-only, 59 fit-only и 73 color-fit. Это 71 товар; 41 товар имеет один variant URL, 22 — пять, семь — восемь и один — три. Sitemap содержит только украинские URL, хотя каждая страница публикует UA/RU/EN hreflang.
- **Crawl evidence:** после последовательной перепроверки все 210 variant URL ответили `200`; metadata/canonical выборка первого прохода охватывает 191 URL, и именно для нее получены 124 уникальных title, 124 description и 67 H1. Для отдельных товаров одна и та же пара title/description повторяется на 5–6 self-canonical variant URL.
...
- **Кодовая причина:** `services.variant_meta.build_variant_meta()` сначала корректно создает комбинированные meta для `color + fit`. Затем `views/product.py` безусловно заменяет `page_title`, `page_description` и `page_keywords` значениями `active_variant_entry.seo_*`, если они заполнены. Эти поля принадлежат цвету и стирают fit-уточнение на обеих color-fit страницах. Одновременно `ProductVariantSitemap` явно включает Cartesian product цветов и активных посадок ради long-tail.
- **Почему это влияет:** sitemap и self-canonical сообщают Google, что каждая комбинация является самостоятельным документом, но ключевые on-page сигналы нескольких документов совпадают. Это размывает URL ownership между color-only и color-fit, повышает вероятность canonical choice со стороны Google, каннибализации и лишнего crawl. Уникальный полный HTML-hash сам по себе не опровергает проблему: его меняют служебные блоки, токены, рекомендации и выбранный контрол, а не обязательно самостоятельная поисковая ценность.
...
- **Безопасное решение:** сформировать allowlist indexable variant URL на основании спроса, ассортимента и уникальной ценности. Для выбранных color-fit страниц meta/H1/описание/изображения/schema должны учитывать оба измерения. Остальные комбинации оставить UX-состояниями и консолидировать на color-only, fit-only или base URL; удалить их из sitemap и SEO-перелинковки, настроить canonical/redirect согласно стабильной карте миграции.
- **Acceptance check:** sitemap содержит только утвержденные self-canonical URL; каждый имеет уникальный intent owner, title/description/H1, фактически выбранные цвет и fit, согласованные изображения/цену/наличие/schema; duplicate-report не находит color-only meta на color-fit URL; удаленные URL не образуют цепочек редиректов.

#### FIND-007 — size path является `index,follow`, хотя canonical ведет на base, а hreflang сохраняет неканонический размер

- **Вердикт:** подтверждено для `/product/classic-tshirt/m/` и кода общего маршрута.
- **Приоритет:** P2.
...
- **Почему это влияет:** canonical — подсказка, а не запрет индексации. Уникальный title вместе с `index,follow` и hreflang на size URL дает противоречивый набор сигналов: страницу можно индексировать, но основным документом объявляется base. Массовые размеры и их комбинации способны вернуть ранее устраненный crawl bloat.
- **Безопасное решение:** выбрать один контракт. Если размер — только UI-состояние, конечный deep link должен быть `noindex,follow` + canonical base без SEO-hreflang либо редиректить на base с сохранением состояния клиентским механизмом. Если размер когда-либо становится индексируемым, он требует самостоятельного спроса, наличия и контента, а не только title. Legacy color resolver должен принимать утвержденный slug или явно отклонять неизвестный формат, а не частично применять запрос.
- **Acceptance check:** все size-only и size-bearing комбинации единообразны; crawler не обнаруживает `index,follow` + non-self canonical + self-like hreflang; legacy redirect сохраняет все валидные выбранные оси или возвращает однозначный ответ.
...
#### FIND-017 — 25 живых 404 имеют внутренние ссылки, хотя не входят в sitemap

- Вердикт: подтверждено завершенным последовательным sitemap + one-hop production crawl.
- Приоритет: P1 для crawl efficiency и UX.
- Evidence: crawl завершен 2026-08-10T21:11:36Z; обработано 1 787 страниц (547 sitemap URL и 1 240 одношаговых внутренних destinations), 1 762 ответа 200, 25 ответа 404 и 15 redirect responses. Все 25 404 имеют живые внутренние ссылки и sitemap_count=0. Маршрут /catalog/custom-print/ имеет 36 inlinks; 24 legacy /product/{33,36,39,42,45,46,48,49,50,51,52,91-106}/ имеют по 23 или 36 inlinks.
- Нюанс custom print: рабочий owner `/custom-print/` отвечает `200`, `index,follow` и self-canonical; stale `/catalog/custom-print/` отвечает `404` и имеет 36 inlinks. Flow/configurator не менять: исправить HTML-ссылки, а узкий 301 добавлять только после проверки backlink/history.
- Почему это влияет: внутренние ссылки ведут crawler и покупателей на тупиковые документы, расходуют обход и снижают качество навигации. Для legacy PDP возможны потеря ссылочного веса, мягкая ошибка в карточках и плохой post-click UX. Это не подтверждение, что все старые ID можно безусловно редиректить: нужен mapping на актуальный published slug.
- Безопасное решение: построить таблицу old ID -> current slug/status; для однозначных соответствий сделать один 301 без цепочек, для удаленных товаров вернуть честный 410/404 и убрать все внутренние ссылки; отдельно исправить /catalog/custom-print/ согласно фактическому рабочему маршруту.
- Acceptance check: 0 linked 404; sitemap не содержит 404; public internal SEO redirects одношаговые; auth/OAuth исключен из этого критерия; custom-print purchase path проходит browser smoke.
...
- Приоритет: P1/P2 вместе с FIND-006; это основание для allowlist, а не требование заполнить контентом все 210 URL.
- Evidence: в публичном scope 78 color variants. VariantDetails существует только для трех записей, но display_name, marketing_html, seo_title, seo_description и seo_keywords пусты во всех; VariantDetailsI18n=0, VariantFAQ=0, ProductOptionProfileI18n=0, VariantCombinationProfile=0. Во всех 78 x 3 контекстах эффективные display/description/marketing/SEO/OG значения разрешаются из base product; у шести RU/EN вариантов срабатывает украинский fallback.
...
- Безопасное решение: сначала утвердить indexable allowlist. Для каждой оставленной страницы заполнить только реальные variant facts, изображение, offer/availability и редакционный ответ на запрос. Остальные состояния сохранить в selector, но удалить из sitemap/SEO rails и консолидировать на назначенного owner. Не генерировать фиктивные тексты, SKU или GTIN.
- Acceptance check: у каждого indexable variant есть запись intent/owner/source и самостоятельный selected-state payload; UI-only комбинации отсутствуют в sitemap и редакционной перелинковке.
...
- Безопасное решение: индексировать color/fit страницу только при наличии правдивого selected-state media или документированной визуальной идентичности. Alt описывает конкретное изображение на языке страницы, а не список ключей; OG и Product JSON-LD используют тот же approved asset.
- Acceptance check: каждый indexable variant имеет корректное выбранное изображение, localized descriptive alt, согласованные hero/OG/schema assets и visual QA; URL без variant media исключен из allowlist.
...
- **Evidence:** base `/product/classic-tshirt/` публикует `@type: Product` с `@id .../#product`. Цветовой `/black/` и color-fit URL публикуют отдельный `Product` и `isVariantOf: {"@id": ".../classic-tshirt/#product"}`. Объекта `ProductGroup` в графе нет; целевой `@id` на base относится к `Product`. Fit-only `/classic/` и `/oversize/` не публикуют `isVariantOf` вообще. На контрольном товаре base и color variant также используют один SKU `TC-1`.
...
- **Безопасное решение:** сначала утвердить индексируемую variant-архитектуру. Затем либо публиковать один canonical Product/ProductGroup с перечисленными реальными вариантами, либо на каждом утвержденном variant URL использовать Product + `isVariantOf` на стабильный ProductGroup, одинаковый `productGroupID`, корректный `variesBy` и уникальные variant identifiers там, где они реально существуют. Не генерировать фиктивные SKU/GTIN.
...
- **Повторяемость:** первый `build_product_seo_block()` действительно диверсифицирован: `71/71` exact-hash уникальны, максимальный pairwise 5-gram Jaccard составляет `0.295` UK, `0.293` RU и `0.327` EN; пар с внутренним диагностическим overlap `>=0.70` нет. Второй `build_landing()` формально также дает разные exact-hash, но имеет 15 пар `>=0.70`; 10 UK/RU и 11 EN пар превышают `0.80`, максимум `0.85/0.85/0.86`. Это наш detector повторяемости, а не порог Google и не доказательство санкции; он показывает, какой именно слой остается boilerplate.
...
- **Почему это влияет:** Google не штрафует страницу просто за длину или два блока. Проблема в совокупности: повторный шаблонный слой размывает главное предложение, создает межстраничное и межъязыковое сходство, публикует противоречивые product facts и ухудшает доверие пользователя. Уникальный HTML-hash не компенсирует слабую самостоятельную ценность.
- **Безопасное решение:** оставить один содержательный editorial-компонент PDP. Базовые факты получать только из проверяемых product/variant полей; не генерировать DTF, плотность, посадку, срок службы или пожертвования, если это не подтверждено для конкретного товара. Удалить публичную фразу про индексацию. Все литералы провести через локализацию, а языковые формы категории/цвета/посадки хранить редакционно вместо склейки.
...
- **Acceptance check:** normalized text hash после удаления header/footer/cards не находит большой SEO-блок на `page>=2`; page 2 остается `200`, indexable, self-canonical и содержит crawlable товары; title/description уникальны; claims первой страницы совпадают с актуальными ценами, доставкой, возвратами и гарантиями.
...
Production evidence насчитывает 73 color×fit URL со слабой самостоятельной ценностью, 836 FAQ rows, сводящихся к 47 уникальным вопросам и 44 ответам, и повторяемые PDP/category boilerplate с geo/commerce claims. Google scaled-content guidance оценивает added value, а не происхождение текста. Нельзя массово создавать city/color/fit pages простыми заменами. Индексируемые страницы должны иметь реальный ассортимент, media, locale-correct copy, variant facts и самостоятельный intent owner; остальные — UI-only noindex/consolidation.
...
**Dependency rule:** CHAIN-001 is the only first release slice because it removes confirmed dead ends without changing URL ownership. CHAIN-004 must precede CHAIN-005/008/009. CHAIN-006/007 must precede mass content or city landing work. No backlink acquisition is recommended before CHAIN-001 through CHAIN-004 are live-verified.
...
|---|---|---|---|
| Общий каталог | /catalog/ и локальные /ru/catalog/, /en/catalog/ | index, self-canonical | Один общий catalog intent; уникальные локализованные H1/meta и навигация |
...
| Базовый PDP | /product/{slug}/ плюс locale paths | index, self-canonical | Опубликованный товар, уникальные product facts, media, Offer и schema |
| Одобренный цветовой landing | /catalog/{category}/{color}/ | index только allowlist | Спрос, достаточный ассортимент, color-specific media, локализованный editorial block и links |
...
| Одобренный thematic landing | `/catalog/theme/{slug}/` плюс locale paths | index только allowlist | Самостоятельный intent, ассортимент, локализация, unique copy/schema и internal links; owner присутствует в thematic sitemap |
| Theme query/filter state, размер, сортировка, multi-filter | query или state URL | noindex, follow и/или canonical owner | UI-only; не размещать в sitemap и editorial SEO links |
...
| Custom print | существующий `/custom-print/` | index, self-canonical | Flow/configurator не перерабатывать; stale `/catalog/custom-print/` убрать из HTML или узко 301 на `/custom-print/` только после проверки истории/backlinks |
...
|---:|---|---|---|---|
| 1 | Удалить/починить 25 внутренних 404, включая stale /catalog/custom-print/ и 24 numeric PDP | P1 crawl, UX, link equity | Проверить каждый ID в production DB и backlink history; custom print flow не менять | Повторный sitemap+one-hop crawl: 0 linked 404; exact successor только one-hop 301 |
| 2 | Утвердить variant allowlist и URL ownership | P1 canonical/crawl/cannibalization | Demand/GSC, inventory, media, locale fact matrix | Sitemap содержит только approved self-canonical owners; удалённые variants имеют согласованный consolidation |
| 3 | Исправить locale content, hreflang и localized schema | P1 relevance/entity | Message catalog, DB translation, hreflang policy | RU/EN visible/schema text не содержит UA fallback; reciprocal self-inclusive hreflang |
| 4 | Оставить один правдивый PDP editorial block и fact policy registry | P1 trust/scaled-content | Owners для material/weight/print/fit/care/shipping/founding facts | Hero, copy, FAQ, schema, llms, feeds и checkout согласованы; служебная SEO-фраза удалена |
| 5 | Нормализовать facets и pagination | P1/P2 crawl efficiency | Internal-link inventory, chosen noindex/canonical strategy | No editorial links на noindex facets; page=1 one-hop clean; page>=2 self-canonical; invalid pages 404 |
| 6 | Добавить clean landing internal links только для approved color/fit owners | P2 discoverability | Steps 2–3 and editorial briefs | Category links ведут на matching locale path landing; query links остаются UX-only |
| 7 | Заполнить variant media, alt и fit data | P1 commerce/image relevance | Production photography, measurements, sellable rows | Every indexable variant has matching hero/gallery, localized alt, availability and dimensions |
| 8 | Исправить ProductGroup/MemberProgram/Offer counts и feeds | P2 structured-data/merchant trust | Variant model and eligibility source of truth | Validator clean; offerCount equals public snapshot; no unsupported MemberProgram type |
| 9 | Исправить mobile filter state and performance bottlenecks | P2/P3 CRO/CWV | Browser regression and consent/analytics contract | Filter badge 0 on clean URL; PDP CLS <0.1 lab median; category LCP and asset budget improve without commerce regressions |
| 10 | GEO/AI monitoring and local policy | P2 long-term factuality | Fact registry, query set, legal confirmation of entity/location | Monthly citation matrix; one truthful Kharkiv/Ukraine narrative; no unapproved city pages |
...
Context7 library IDs queried: /websites/developers_google_search and /websites/developers_google_search_appearance_structured-data. Official guidance used: faceted navigation crawl management; reciprocal/self-inclusive hreflang and x-default; ProductGroup/product variants; structured-data/visible-content correspondence; LocalBusiness factuality; Google AI feature technical requirements; and loyalty-program supported types. No third-party marketing claim was treated as a Google requirement.
