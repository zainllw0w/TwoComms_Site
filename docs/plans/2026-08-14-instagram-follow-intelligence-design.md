...
- [ ] Remaining gates: disposable-MariaDB concurrency/race proof, the complete viewport/zoom/accessibility matrix, a consented read-only Graph follow probe, auto-award calibration/shadow evidence, post-issuance linked-order refund/revoke policy, and deployment-log reconciliation. Core local reply/checkout/operations tests are green; feature SHA `04d392faa` is deployed with refreshed production migration, InnoDB, daemon/runtime, empty-queue, duplicate, promo-shape, ambiguous-delivery, and HTTP evidence in the detailed checklist.
