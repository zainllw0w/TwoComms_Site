...
- [ ] Remaining gates: disposable-MariaDB concurrency/race proof, the complete viewport/zoom/accessibility matrix, a consented read-only Graph follow probe, auto-award calibration/shadow evidence, post-issuance linked-order refund/revoke policy, and deployment-log reconciliation. Core local reply/checkout/operations tests are green; prior main integration, production migration, daemon health, and read-only invariants are evidenced below, while the current edge patch still requires a production SHA refresh.
