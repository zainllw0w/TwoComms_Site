...
- [x] Current production deployment and read-only runtime gate. Evidence: runtime code SHA `f476223a25321d21b3af1feeff3c958b9da6722c` was verified before the subsequent docs-only closeout; migrations `0166` and `0095` are applied; all ten follow/UGC tables are InnoDB with expected unique indexes; daemon heartbeat is fresh on `instagram_login` with polling disabled; public and management health/auth boundaries return expected HTTP statuses; no synthetic message or test-event marker was created.
