...
- [x] **3.1** Add a rendered locale matrix test for standard catalog/PDP pages that fails when RU/EN title, H1, main editorial content, critical commerce UI, FAQ or JSON-LD remains Ukrainian, except approved brand names, SKUs and proper nouns. Exclude Custom Print from this matrix; do not fail on every isolated borrowed word or decorative asset.

  - Code/test commit: `8a3be6f2f4d4d7279c5619e1778d7310e6fdff37` (`fix(seo): localize shared catalog and telegram shell`) added the rendered RU/EN matrix for standard catalog/PDP shared Telegram/profile shell, root catalog filters and Organization slogan. Only the corresponding RU/EN gettext entries and compiled `.mo` files were changed; the independent `seo_utils.py` and `.serena/` worktree changes were excluded.
  - TDD/local proof: the new matrix first failed with Ukrainian Telegram/profile/filter/slogan fallbacks. After the minimal PO translations and `msgfmt` compilation, `storefront.tests.test_rendered_locale_matrix` passed `7/7`; the rendered locale plus Smart Selector suite passed `61/61`; `manage.py check`, touched-file compilation and `git diff --check` passed. Custom Print configurator payload/cache no-submit checks passed `2/2`; its unrelated legacy CTA expectation remains outside this slice and no Custom Print file was edited.
  - Context7/Django 5.2 verification: the documented active-language/template rendering and language-aware fragment cache contracts were used; no manual URL-language concatenation or global cache bypass was introduced.
  - Production proof: production pulled and restarted at SHA `8a3be6f2f4d4d7279c5619e1778d7310e6fdff37`; `manage.py check` and `/healthz/` returned successfully. Live `/ru/catalog/`, `/en/catalog/`, `/ru/product/classic-tshirt/black/` and `/en/product/classic-tshirt/black/` returned `200` with matching `Content-Language`, localized filter/Telegram/profile/slogan strings and no corresponding Ukrainian fallback markers. The browser automation service was unavailable because its existing session lock could not be acquired; curl/live HTML plus server-rendered regression evidence are recorded instead.
  - Boundary: no DTF route, subdomain, module or blog; no Custom Print route, configurator, product data, pricing, media, cart, analytics or submission behavior; no ranking, traffic, citation, rich-result or conversion uplift is claimed. Full catalog/PDP browser language-switch and remaining untranslated product-field audit continue under `3.4`, `3.5` and `3.6`.
...
- [x] **5.5d.3** Align canonical, Open Graph, Twitter and structured-data ownership for paginated catalog UI states. Clean and tracking-only page N keep the truthful same-locale page-N owner. Facet, sort and root-category page N keep their filtered pagination/cache identity for users, but consolidate to the clean same-locale route without an ordinal `page` and omit `CollectionPage`; `BreadcrumbList` remains. Thematic color state uses the same schema-suppression contract.

#### Task 5.5d.3 production evidence

- Code/test commit: `ab4cca840b58406b84e07f6e02c0577bdfc73469` (`fix(seo): align catalog facet pagination owners`) was pushed to `origin/main`, pulled fast-forward on production and activated with `tmp/restart.txt`. Production `manage.py check` completed with no system-check errors and the server reported the exact release SHA.
- Root cause: a noindex facet page such as `?color=black&page=2` rendered a different product slice from clean page 2, but canonical, `og:url`, `twitter:url` and `CollectionPage @id/url` all claimed the clean page-2 document. The release introduces one `catalog_owner_path`/`catalog_facet_state` context contract, keeps page-N ownership only for clean or tracking-only states, and prevents a filtered `mainEntity` from being published under another document's `CollectionPage @id`.
- TDD/review gates: the focused owner test first produced six expected failures proving that homogeneous fixtures could not distinguish fit/sort slices; mixed fit and price fixtures then made clean, color, fit, sort and root-category page-2 slices deterministic. The final targeted suite passed `57/57`; the stable catalog, Smart Selector, merchandising, locale and thematic gate passed `243/243`; Django check, catalog template compilation, touched-file `py_compile` and `git diff --check` passed. Two independent final reviews reported no Critical or Important findings. A broader exploratory 287-test combination exposed existing stale swatch expectations and cross-module locale order dependence outside this five-file diff, so it was not used as release evidence.
- Context7/Django 5.2.6 verification: `QueryDict` key iteration is suitable for detecting the presence of a validated facet axis, while `lists()`/multi-value APIs are required when retaining repeated values. The existing pagination serializer continues to preserve validated multi-value facet state and exclude external tracking state; the new owner predicate only classifies whether a non-page catalog key is present.
- Live UK/RU/EN matrix: color, fit, `sort=price-asc` and root `category=tshirts` page-2 states returned `200 + noindex`, zero SEO hreflang, the clean same-locale owner in canonical/OG/Twitter, one `BreadcrumbList`, zero `CollectionPage`, and facet-preserving previous-page links. Clean T-shirt page 2 returned `200 + index`, four hreflang links, a self-canonical page-N owner and a matching `CollectionPage`; tracking-only page 2 returned `noindex`, the same truthful page-N owner and product slice, no SEO hreflang, and no tracking propagation into pagination links. Repeated clean/facet requests retained identical owner, robots, product and schema contracts.
- The live T-shirt evidence remains materially distinct: black page 2 exposes 8 product links while clean page 2 exposes 14; `sort=price-asc` changes ordering; root `category=tshirts` differs from clean root page 2. `Content-Language` is correct for UK/RU/EN and no sampled route emits `X-Robots-Tag`.
- Thematic production has only one valid `streetwear` page. Clean page 1 remains the UK-owned `CollectionPage`; its RU/EN fallbacks remain `noindex` under the existing locale-publication policy. Filtered color page 1 is `noindex` with no `CollectionPage`, and out-of-range page 2 correctly returns `404`. Local fixtures separately cover truthful thematic page-N ownership when a second page exists.
- Adjacent P1 finding retained, not hidden by this checkbox: clean root `/catalog/?page=2` is still `index, follow` and self-canonical but renders three category showcase cards and zero visible product cards while JSON-LD claims 16 products. Broad 5.4 was therefore reopened and 5.4b remains the next implementation slice.
- Scope boundary: no DTF subdomain/blog/module/route, Custom Print SEO or configurator, product copy, inventory, media, cart or analytics persistence was changed. This checkpoint claims signal and schema consistency only; no ranking, traffic, crawl-budget or conversion uplift is claimed. Parent `5.5d`, `5.5` and Task 5 remain open pending the root-catalog fix plus access-log/GSC evidence.
