...
| 2026-05-10 | bug | `28860212` | Bugfix — catalog AVIF/WebP `<picture>` пустой для color-images с кириллическими файлами. Root cause: `FileField.url` percent-encodes path (`апр.` → `%D0%B0%D0%BF%D1%80.`), а `optimized_image`/`preload_image` template-tags строили `base_name` из encoded URL → `Path.glob` не матчил on-disk варианты (хранятся в Unicode). Fix: `urllib.parse.unquote(image_path)` в обеих функциях. ASCII пути не затронуты (no-op). Бонус: regenerated 18 missing variants через `manage.py optimize_images --steps=color`. Также Phase 14-regression fix `6406013f`: ветка `preferred_image_url` рендерила raw `<img src>` вместо `optimized_image` — теперь корректно отдаёт AVIF srcset. |
| 2026-05-10 | 19g | `ceb795b9` | Phase 19g — два catalog SEO-апгрейда. **(1) Colour-aware long-form SEO copy**: новый сервис `services.color_seo_copy.build_catalog_color_seo(category, selected_color_slugs, available_colors)` возвращает `{h2, paragraphs, queries}` для трёх состояний — `/catalog/` root (brand-level landing, ссылки на 3 категории + 5 топ-цветов), `/catalog/?color=<slug>` (cross-category copy для черного/кайот/оливы/серого/белого + generic-fallback для остальных slugов), `/catalog/<cat>/?color=<slug>` (category × colour). Каждое состояние несёт **HF/MF/LF query chips** с класс-маркерами `catalog-color-seo__chip--{hf,mf,lf}` (orange / blue / neutral). Для не-курированных slugов — templated copy с label из chip-data + 7 chip-запросами. Партиал `partials/catalog_color_seo.html`, CSS `.catalog-color-seo*` дописан в `category-seo-blocks.css`. Per-category /catalog/<cat>/ без фильтра возвращает `None` — не дублирует `category.description`. **(2) Live showcase swatches**: `views/catalog._compute_showcase_swatches(category_ids, fallback)` — single GROUP BY query по `ProductColorVariant` + `Count('id')`, top 4 hex по usage; padding из legacy-палитры если в категории <4 цветов. Hard-coded `swatches` в `CATALOG_SHOWCASE_CARD_CONFIG` теперь fallback. На проде показывает реальные `#000000 → #fafafa → #B27815 → #F7A1B9 → #C2EBE5` вместо легаси `#050505/#6a6b60/#e7e1d3/#8c8f79`. Tests: 11 (`test_phase19g_color_seo`). Регрессия Phase 9/10/10b/11/14 + catalog + general_catalog + backfill = **85/85 OK**. Smoke prod: catalog-color-seo на /catalog/ + /catalog/?color=black, все 3 freq-маркера присутствуют. |
| 2026-05-10 | 19f | `5d664450` | Phase 19f — три catalog UX-фикса. **(1) Bottom SEO block на /catalog/ root**: per-category catalogs (hoodie/tshirts/long-sleeve) рендерили tab-блок (top_menu/top_filters/top_queries) ещё с Phase 10b, общий `/catalog/` — нет (gate `{% if category %}`). Новый сервис `services.general_catalog_seo.get_general_catalog_seo_layout` синтезирует in-memory layout (top_menu=все категории, top_filters=available_colors, top_queries=14 курированных) совместимый с `partials/category_seo_blocks.html`. Template guard → `category_seo_layout.has_any`. **(2) Color chips на /catalog/ root**: `partials/color_filter_chips.html` жил внутри `{% if not show_category_cards %}` — на корневом catalog скрывался. Вынес наружу — chips на каждом catalog-экране. **(3) Blur on color filter**: `build_color_preview_map` ставил в `first_image_url` 320px-thumbnail (`_320w.webp`); `attach_preferred_card_image` подавал его в `optimized_image` template-tag, который ищет sibling-варианты по stem (`<stem>_<W>w.{avif,webp}`) — для `foo_320w.webp` ничего не находил → fallback на raw `<img src>` 320px stretched до ~480px = blur на Retina. Fix: добавил `original_image_url` в preview-dict (un-optimized source), `attach_preferred_card_image` предпочитает его — теперь `optimized_image` находит полный AVIF/WebP srcset (320→1024w на проде проверено). **(4) Phase 18 leaky comment**: 6-строчный `{# ... #}` в `product_card.html` рендерился как текст (multiline `{# #}` — известный Django-баг, см `79ad7f25`) — текст утекал на каждую карточку каталога/главной. Свернул в одну строку. Tests: 12 (`test_general_catalog_seo` — service + view + template). Регрессия Phase 9/10/10b/11/14 + catalog + backfill = **85/85 OK**. |
| 2026-05-10 | bug | `4f90628a` | Bugfix — фильтр по чёрному цвету: (1) PDP открывался на «Кайот» вместо чёрного при клике с `/catalog/?color=black`, (2) 49 из 65 товаров не было видно ни в одном цветовом фильтре. Root causes: (1) `partials/product_card.html` строил ссылку через `{% url 'product' p.slug %}` без path-сегмента цвета — PDP не знал контекста и брал первый вариант в DB-порядке. (2) Большинство legacy-товаров не имели ни одного `ProductColorVariant` — Phase 9 фильтр `?color=` join'ил через `color_variants` и они проваливались. **Fix 1**: `attach_preferred_card_image` уже выставлял `preferred_color_slug`; шаблон теперь строит `{% url 'product' p.slug p.preferred_color_slug %}` когда slug != ''. **Fix 2**: новая команда `manage.py backfill_default_color [--dry-run] [--include-drafts]` (resolve `Color` через primary_hex='#000000' с Python-fallback на name__icontains для SQLite-test compat). Создаёт 1 `ProductColorVariant(color=Чорний id=25, is_default=True, order=0, slug='black')` для каждого published-товара без вариантов. Без images — gallery fallback на `product.main_image`. Tests: 8 (`test_backfill_default_color`) + регрессия 22/22 (Phase 9 + 14). Prod result: видимых под `?color=black` товаров **16 → 61** (4 не показаны — у них только не-чёрные варианты, корректно). |
| 2026-05-10 | 19c | `68c0f574` | Phase 19c — lazy-load below-fold PDP CSS (`product-seo-landing.css`, `category-seo-blocks.css`). Оба стайлшита применяются только к контенту в самом низу страницы (Phase 15 SEO-блок между FAQ и футером). Переключены на pattern `media="print" onload="this.media='all'"` + `<noscript>` fallback. **Mobile Lighthouse median (3 runs) до 19b/c → после: LCP 6660ms → 3969ms (−2691ms / −40%)**, FCP стабильно 2.1s, perf score 71→81, CLS 0, TBT 92ms. Главный механизм: lazy CSS освободил critical-render path → браузер быстрее dispatch'ит preload AVIF hero. **Гача**: первый билд использовал многострочные `<link>`-теги — Django HTML-минификатор сжимал их в одну строку без media/onload атрибутов; пришлось схлопнуть теги вручную (commit `68c0f574`). |
| 2026-05-10 | 19b | `dffc376f` | Phase 19b — idle-load `analytics-loader.js` вместо `<script defer>`. Loader (~55 KB) внутри сам уже использует `requestIdleCallback` для GTM/Pixel/Clarity, но **парсинг 55 КБ исходника** всё равно происходил во время defer-окна и накручивал TBT. Заменил `<script defer src=...>` на inline-IIFE: `requestIdleCallback(inject, { timeout: 4000 })` создаёт `<script async>` динамически. Safari (нет rIC) — fallback на `setTimeout(load_listener+1500ms)`. Network preload `as=script fetchpriority=low` держит файл в HTTP-кэше для мгновенной инъекции. Эффект сложно вычленить от 19c (мерили совместно). |
| 2026-05-10 | 19a | `83531ae8` | Phase 19a — drop FontAwesome on PDP. `pages/product_detail.html`: `{% block fontawesome_css %}{% endblock %}` (override). Grep по rendered HTML: 0 fa-* классов. Lighthouse mobile audit показывал `vendor/fontawesome/css/all.min.css` 99.7% unused (~21.5 KB / 21.6 KB). Удалена загрузка → -21.6 KB wire bytes на каждом PDP-запросе, unused-css-rules savings: 450ms → 180ms (-270ms). Variance simulate-throttling 3 runs: LCP median 6.7s (range 3.9–7.4s — high noise), TTI 8.4s, TBT 82ms. Дальнейшие architectural wins (route-split CSS bundle, Bootstrap subset, GTM/Pixel TBT) отложены — требуют большого рефакторинга. |
