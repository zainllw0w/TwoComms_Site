...
| **Критичні знахідки з deep research (2026-05-20)** | 🆕 Підтверджено з авторитетних джерел: (1) `ProductGroup` + `hasVariant` schema — обов'язкова для apparel з 2024 (Google Search Central Blog); (2) `MerchantReturnPolicy` на Organization — прямий фактор Merchant Center trust; (3) `MemberProgram` structured data — новий сигнал для loyalty benefits у Search; (4) `OfferShippingDetails` в кожному Offer — критично для Merchant Listings eligibility; (5) FAQPage rich results **депрековані** для e-commerce з серпня 2023 — переробити на UX/semantic, а не SERP-feature; (6) `dateModified` відсутній — AI-моделі та Perplexity не можуть оцінити freshness; (7) GBP SAB для чистого online-магазину **нелегітимний** без фізичної точки — потрібен гібридний шоурум або партнерська точка видачі. |
...
> [!IMPORTANT]
> **Підтверджено deep research (Netpeak/Promodo фреймворк):** Навіть якщо повний переклад опису товару недоступний, **Title, Description, H1 та характеристики** (колір, розмір, виробник) ПОВИННІ бути локалізовані. Це дає Google достатній сигнал унікальності та запобігає 100% дублюванню. Для неперекладених позицій рекомендується шаблонна генерація унікального абзацу на основі характеристик мовою цільової локалі (наприклад: *"Купить [Название товара] в Харькове по цене... Характеристики: ..."*).
>
> **Hreflang стратегія:** Якщо сторінка є повним дублем без жодної локалізації, її **не можна** додавати до матриці `hreflang`. Використовуйте `noindex, follow` на таких сторінках та прибирайте їх із мовних файлів `sitemap.xml`. Після додавання перекладу — знімайте `noindex` та додавайте в `hreflang` кластер. Це підтверджено офіційною документацією Google (https://developers.google.com/search/docs/specialty/international/localized-versions).
...
> [!IMPORTANT]
> **Deep research підтвердження (SSR vs AJAX):**
> - JSON-LD розмітка `AggregateRating` **та** блок відгуків у UI повинні рендеритися серверно (SSR/SSG). Googlebot використовує двохетапний краулінг — спочатку сирий HTML (перша хвиля), рендеринг JS — пізніше (затримка до 2 тижнів для глибоких PDP).
> - Якщо розмітка в HTML, а текстові відгуки підвантажуються AJAX — Google побачить structured data, але не знайде фізичного тексту відгуків. Це порушує правило відповідності контенту.
> - **ProductGroup агрегація:** Для варіацій одного товару (5 кольорів) використовувати `aggregateRating` на рівні батьківської `ProductGroup`. Тоді кожен колір у видачі отримає сумарні зірочки, підвищуючи CTR менш популярних варіантів.
> - **Ризик Manual Action:** Заборонено дублювати загальний рейтинг магазину (наприклад, з Google Maps) у розмітку `aggregateRating` кожного окремого товару — це прямий шлях до санкцій *"Spammy structured data"*.
...
> [!WARNING]
> **Deep research оновлення (GBP SAB ризики):**
> - Згідно з офіційними інструкціями Google, **суто онлайн-магазини (pure e-commerce) НЕ мають права** на створення профілю в Google Business Profile.
> - **Service Area Business (SAB)** призначений виключно для бізнесів із особистим контактом на території клієнта (сантехніки, клінінг, доставка їжі власною кур'єрською службою).
> - З 2023-2024 років Google вимагає від нових профілів в Україні **відеоверифікації** — потрібно записати на відео вивіску, робоче місце, склад. Online-магазин з квартири не пройде цей крок.
> - SAB-профілі **не мають доступу** до Local Inventory Ads (LIA) — головної цінності GBP для e-commerce.
>
> **Легітимний обхідний шлях — «Гібридний Шоурум»:**
> 1. Орендувати куточок у дружньому офлайн-бізнесі (ательє, шоурум, склад логістичного партнера).
> 2. Встановити стійку з вивіскою бренду + терміналом оплати.
> 3. Створити GBP з реальною адресою, реальними фото для верифікації та обмеженими годинами роботи.
> 4. Це дозволяє легально підключити LIA — товари починають ранжуватися за геозапитами («купити худі Харків»), приріст CTR у мобільній видачі до 35%.
...
- 🛠️ TASK I: «SEO Indexation Upgrade: індексація 2-сегментних варіацій (Колір + Посадка)» (2-3 дні)

---

## 12. Глибокий аналіз унікальності та індексації товарних варіацій (2026-05-20)

### 12.1. Оцінка поточного підходу (Scoring)

*   **Індексація варіацій (URL-структура): 7.5 / 10**
    *   *Чому:* Система надійно захищає від сміттєвого краулінгу розмірів (всі розмірні URL склеюються канонікалом на базу). Проте, через занадто агресивну канонізацію ми повністю втрачаємо комбіновані низькочастотні запити з високим комерційним інтентом (наприклад, *"купити чорну футболку оверсайз"*). Поточний canonical для 2+ сегментів ріже ці сторінки з індексу Google, змушуючи базову сторінку конкурувати за неспецифічними запитами.
*   **Унікальність контенту варіацій: 8.0 / 10**
    *   *Чому:* Завдяки динамічному блоку перелінковки (US-6) та інтеграції специфічних для фіту абзаців (`FIT_SEO_COPY`) сторінки варіацій мають гарну текстову відмінність. Проте, загальний опис принту/дизайну та шаблонні блоки бренду (DTF, підтримка ЗСУ) дублюються приблизно на ~65-70%. Google лояльний до такого дублювання в e-commerce, але для ідеального результату відсоток схожості контенту треба зменшити.

### 12.2. Структура товарних варіацій та роутинг

Каталог використовує правильну реляційну схему: базова модель `Product` (дизайн принту) об'єднує кольорові варіанти `ProductColorVariant` та посадки `ProductFitOption`.
З боку URL-адрес реалізовано динамічний роутинг `/product/<slug>/<v1>/<v2>/<v3>/`, де сегменти розбираються вмістовно (content-addressable):
*   `/product/225-tshirt/` — базова сторінка (self-canonical) -> **Індексується**
*   `/product/225-tshirt/black/` — 1 сегмент колір (self-canonical) -> **Індексується**
*   `/product/225-tshirt/oversize/` — 1 сегмент посадка (self-canonical) -> **Індексується**
*   `/product/225-tshirt/black/oversize/` — 2 сегменти колір+посадка (canonical -> `/product/225-tshirt/`) -> **НЕ індексується**
*   `/product/225-tshirt/black/m/oversize/` — 3 сегменти колір+розмір+посадка (canonical -> `/product/225-tshirt/`) -> **НЕ індексується**

### 12.3. Вирішення проблеми комбінованих запитів (наприклад: "купити чорну футболку оверсайз з принтом")

Для вирішення завдання унікальної індексації комбінованих товарів без ризику потрапляння під санкції за дублі або перевищення ліміту краулінгу пропонується наступне архітектурне рішення:

1.  **Дозволити індексацію 2-сегментних URL (Колір + Посадка)**:
    *   Сторінка виду `/product/<slug>/<color>/<fit>/` (де вибрано один колір та одну посадку, але БЕЗ розміру) повинна бути `self-canonical` (`is_self_canonical = True`).
    *   Це дає точне цільове ранжування для запитів на кшталт *"чорна футболка оверсайз з принтом [назва]"*.
    *   Кількість додаткових сторінок для 65 товарів становитиме близько 300–400 унікальних URL. Це безпечний об'єм, який швидко індексується Googlebot.
2.  **Заборонити індексацію розмірів (жорстке склеювання)**:
    *   Будь-який URL, що містить сегмент розміру (наприклад, `/product/<slug>/<color>/<fit>/m/` або `/product/<slug>/m/`), повинен канонікалізуватися на відповідний URL **без розміру** (наприклад, на `/product/<slug>/<color>/<fit>/` або на базований товар). Розміри не мають окремого пошукового попиту та створюють гігантське сміттєве навантаження на роботів.

### 12.4. Дорожня карта впровадження та зниження дублювання контенту

#### Етап 1: Оновлення канонічної логіки та мета-тегів
*   **Зміна правил в `variant_meta.py`**:
    *   Якщо `segments_count == 2` і при цьому присутні і `color_slug`, і `fit_code` (а `size_code` відсутній) -> встановити `is_self_canonical = True` та `canonical_path = current_path`.
    *   Якщо присутній `size_code` -> canonical завжди веде на URL без розміру (наприклад, на `/product/<slug>/<color>/<fit>/` або `/product/<slug>/<color>/` тощо).
*   **Оптимізація Title та Description**:
    *   Для комбінованих сторінок генерувати точні мета-теги:
        *   **Title**: `Футболка {title} — {color}, оверсайз крій | TwoComms`
        *   **Description**: `{color} футболка «{title}» оверсайз крою від українського бренду TwoComms. Надійний DTF-друк, щільна бавовна, доставка з Харкова за 1-3 дні.`

#### Етап 2: Збільшення унікальності тексту на сторінках варіацій
Для того, щоб Google вважав кожну варіацію окремим якісним товаром:
1.  **Комбінований SEO-блок в `product_seo_landing.py`**:
    *   При рендерингу комбінації Колір + Посадка зшивати унікальні абзаци для конкретного кольору (із внутрішнім лінком на інші кольори) + абзац з описом посадки (пояснення лекал оверсайзу чи класики) + гео-абзац, адаптований під цей силует.
2.  **Шаблонізація синонімів (Spin-writing)**:
    *   Для загального опису бренду впровадити 3-4 варіанти речень, які випадково або на основі хешу від URL обираються для рендерингу (це знизить схожість тексту між варіаціями з 70% до 30-40%).
3.  **Візуальна унікальність**:
    *   Переконатися, що на `/product/<slug>/<color>/<fit>/` першим зображенням у галереї, Open Graph та JSON-LD структурованих даних підставляється саме фото відповідного кольору (реалізовано у Phase 21 для кольорів, потрібно поширити на комбінації).

#### Етап 3: Пошукова видимість
*   **Карта сайту (`sitemaps.py`)**:
    *   Розширити `ProductVariantSitemap` для автоматичного додавання 2-сегментних комбінацій (Колір + Посадка) до XML карти сайту, щоб Googlebot миттєво дізнався про нові посадкові сторінки.

---

## 13. Підтверджені знахідки з Deep Research (2026-05-20)

> Цей розділ містить верифіковані рекомендації з двох глибоких досліджень, проведених 2026-05-20. Кожна рекомендація підтверджена авторитетними джерелами (офіційна документація Google, Netpeak, Semrush, Ahrefs, кейси українських брендів).

### 13.1. 🆕 CRITICAL — `ProductGroup` + `hasVariant` мікророзмітка (підтверджено: Google Search Central Blog, лютий 2024)

**Джерело:** https://developers.google.com/search/blog/2024/02/product-variants

Поточна Product schema трактує кожен варіант як ізольований об'єкт. Це руйнує здатність Google кластеризувати варіації під одним батьківським товаром.

**Що потрібно:**
- Обернути PDP-схему у `ProductGroup` з полями `productGroupID`, `variesBy`, `hasVariant`.
- Кожна варіація (`Product`) у `hasVariant` повинна мати унікальний `sku`, `color`, `size`, `image` та власний `offers.url`.
- `variesBy` для TwoComms: `["https://schema.org/color", "https://schema.org/size", "https://schema.org/sizeGroup"]` (sizeGroup = fit/посадка).

**JSON-LD приклад для TwoComms:**

```json
{
  "@context": "https://schema.org/",
  "@type": "ProductGroup",
  "productGroupID": "225-tshirt",
  "name": "Футболка 225ОШП",
  "description": "Футболка з DTF-принтом на підтримку 225 окремого штурмового полку",
  "url": "https://twocomms.shop/product/225-tshirt/",
  "brand": {
    "@type": "Brand",
    "name": "TwoComms"
  },
  ...
  "hasVariant": [
    {
      "@type": "Product",
      "sku": "225-TSHIRT-BLACK-M-CLASSIC",
      "name": "Футболка 225ОШП — чорна, M, класичний фіт",
      "color": "Чорний",
      "size": "M",
      "image": "https://twocomms.shop/media/products/225-tshirt-black.webp",
      "url": "https://twocomms.shop/product/225-tshirt/black/",
      "offers": {
        "@type": "Offer",
        "price": "788.00",
        "priceCurrency": "UAH",
...
**Прогноз імпакту:** Консолідація ranking-сигналів між варіаціями → +10-20% позицій для PDP з 2+ кольорами.

---

### 13.2. 🆕 CRITICAL — `MerchantReturnPolicy` на Organization (підтверджено: Google Merchant Listing docs)

**Джерело:** https://developers.google.com/search/docs/appearance/structured-data/return-policy

Google вимагає `MerchantReturnPolicy` під Organization для e-commerce. Для TwoComms це особливо важливо через DTF-друк (non-refundable custom prints vs standard items).
...
Для кастомних DTF-принтів потрібне окреме переозначення на рівні `Offer` з `returnPolicyCategory: MerchantReturnNotPermitted`.

---

### 13.3. 🆕 HIGH — `MemberProgram` structured data (підтверджено: Google Developer docs, 2026)

**Джерело:** https://developers.google.com/search/docs/appearance/structured-data/loyalty-program

На PDP вже видно бонуси/бали, але schema `MemberProgram` відсутня. У 2026 це прямий шлях до merchant knowledge panels.
...
```json
{
  ...
  "hasMemberProgram": {
    "@type": "MemberProgram",
    "name": "TWOCOMMS Бали",
    "description": "Бали за покупки, обмін на промокоди або донати на ЗСУ.",
...
### 13.4. 🆕 HIGH — `OfferShippingDetails` у кожному Product Offer (підтверджено: Google Merchant Listing)

**Джерело:** https://developers.google.com/search/docs/appearance/structured-data/merchant-listing

Поточний `Offer` не містить `shippingDetails`. Без цього Google не показує повну shipping інформацію у Merchant Listings.
...
### 13.5. 🆕 HIGH — `dateModified` для AI freshness scoring (підтверджено: Google AI Features docs)

**Джерело:** https://developers.google.com/search/docs/appearance/ai-features

Perplexity та Gemini пріоритезують джерела з високим рівнем часової свіжості. На PDP відсутній:
- Видимий timestamp «Останнє оновлення: ...» біля основного контенту.
- Атрибут `dateModified` у JSON-LD Product/WebPage.

Для streetwear бренду з лімітованими дропами це критично: без temporal signals алгоритм віддасть перевагу конкуруючим агрегаторам.

**Fix:** Додати `dateModified` у WebPage schema та видимий текст на сторінці.

---

### 13.6. 🆕 HIGH — FAQPage Rich Results ДЕПРЕКОВАНІ для E-commerce (підтверджено: Google, серпень 2023)

**Джерело:** https://developers.google.com/search/blog/2023/08/howto-faq-changes

Google офіційно обмежив FAQ rich results лише для урядових та health-сайтів. Для звичайних e-commerce магазинів FAQ-сніпети **більше не показуються** регулярно.

**Що це означає для TwoComms:**
- Поточні 14 FAQ на PDP + 30 на /faq/ — **залишити** як UX-контент і semantic signal для AI-парсерів.
- **НЕ розраховувати** на FAQ rich results у SERP як тактику підвищення CTR.
- Переорієнтувати FAQ-зусилля на `QAPage` schema замість `FAQPage` для інструкційних сторінок (догляд за одягом, розмірна сітка).

---

### 13.7. 🆕 MEDIUM — Розподіл PageRank: кнопки замість посилань для розмірів (підтверджено: Google docs)

Поточна реалізація використовує `rel="canonical"` для склеювання розмірних URL. Додатково рекомендується:

1. **Елементи вибору розміру** (XS, S, M, L) мають бути `<button>` або `<div>` з `onclick` на JS, а не `<a href>`. Googlebot не бачить посилань для переходу → вага сторінки не розпорошується.
2. **`rel="nofollow"` НЕ працює** як бар'єр з 2019 року — Google сприймає його як підказку, не директиву. Роботи все одно перейдуть за посиланнями.
3. **Хлібні крихти** на сторінці та в BreadcrumbList мають бути **статичними** та вести до канонічної категорії.
4. **Мульти-BreadcrumbList** — Google дозволяє кілька незалежних ланцюжків на одній сторінці (наприклад, через основну категорію + колекцію).

---

### 13.8. 🆕 MEDIUM — Чіпси «Часті пошуки» — ліміти та безпека (підтверджено: Ahrefs, Semrush, SpamBrain docs)

Блоки швидких посилань (чіпси/теги) на PLP — потужний метод перелінковки, але хаотичний запуск створює ризик SpamBrain санкцій.

**Безпечні ліміти:**
- **PLP (категорійна):** max **12-15 чіпсів**, сформованих на основі реального пошукового попиту (50+ запитів/міс).
- **PDP (товарна):** max **3-5 посилань** лише на вузькі теги, що безпосередньо характеризують товар (бренд, матеріал).
- **Контроль In-Degree:** Сторінка фільтра не повинна отримувати посилання з усього сайту. Обмежувати лінкінг межами поточної гілки категорій.
- **Анкорна диверсифікація:** Використовувати шаблонізатор для генерації різних анкорів (Варіант 1: «Худі чорні TwoComms», Варіант 2: «Чорні худі від TwoComms», Варіант 3: «TwoComms худі чорного кольору»).

---

### 13.9. 🆕 MEDIUM — Узгодження Merchant Center XML-фіду з органічним SEO (підтверджено: Google Merchant docs)

**Проблема:** Невідповідність ціни/наявності між XML-фідом та цільовою сторінкою — найчастіша причина блокувань у Google Merchant Center.

**Рішення:**
1. **Атрибут `canonical_link`** у XML-фіді: в `link` вказуємо URL варіації, в `canonical_link` — чистий SEO-URL. Google використовує `canonical_link` для передачі ваги в органіці.
2. **SSR обов'язковий**: GET-параметри URL повинні рендеритися серверно (ціна, мета-теги, JSON-LD конкретної варіації — до відправки клієнту). Інакше — *"Price Mismatch"* блокування.
3. **`ProductGroup`** у JSON-LD: бот Merchant Center знаходить відповідний `hasVariant` за ключем `url`, звіряє ціну та наявність.

---

### 13.10. 🆕 MEDIUM — AI Overviews в Україні — поточний стан (підтверджено: Google, травень 2025)

Блок «Огляд від ШІ» (AI Overviews) офіційно доступний в Україні з **травня 2025**. Генерується українською мовою.

**Фактори потрапляння товарів:**
1. **Merchant Center Free Listings** — товар ПОВИНЕН бути активований у програмі безкоштовних оголошень Google.
2. **Data Consistency** — будь-яка розбіжність у ціні, валюті (UAH) чи наявності між XML-фідом, JSON-LD та текстом на сторінці = виключення з AI-блоків.
3. **GEO-оптимізація** — PDP має містити розгорнутий опис характеристик, Q&A блок, температурні режими, вологостійкість тощо.

**Щодо llms.txt:**
- Googlebot **НЕ використовує** файл `llms.txt` для AI Overviews (спирається на класичний пошуковий індекс).
- ChatGPT Search, Perplexity та Claude **активно використовують** `llms.txt` для швидкого зчитування структури.
- Директива `Google-Extended` у robots.txt блокує навчання Gemini, але **НЕ блокує** показ у AI Overviews.

---

### 13.11. 🆕 LOW — pSEO тематичних лендінгів — вимоги до унікальності (підтверджено: Google Helpful Content, Core Update 2024)

Для безпечного масштабування посадкових сторінок фільтрів:

1. **LLM-генерація тексту:** 2-3 абзаци унікального тексту на основі технічних даних (характеристики бренду, рейтинг товарів у вибірці). Перевірка на спам-маркери.
2. **Data-driven блоки:** Ціновий діапазон, таблиці порівняння топ-3 моделей, локальні дані про наявність.
3. **UGC агрегація:** Виводити 2-3 останніх відгуки до товарів з вибірки фільтра.
4. **Мінімальний поріг:** НЕ відкривати для індексації сторінки фільтрів з менше ніж **3 активними товарами**. Якщо товарів мало → `noindex, follow`.
5. **Темп публікації:** Max **100-300 сторінок на тиждень**. Відстежувати % індексації в GSC.

---

### 13.12. 🆕 MEDIUM — `availability=MadeToOrder` блокує Google Shopping (підтверджено: Vilni v2 аудит + Google Merchant docs)

**Джерело:** Vilni AI v2.0 аудит від 2026-05-20, підтверджено Google Merchant Center Best Practices

На **кожному** товарі TwoComms поле `availability` у Offer = `https://schema.org/MadeToOrder`. Це технічно коректно (DTF-друк відбувається після замовлення), але:

- Google Shopping tab **пріоритезує** товари з `InStock` для запитів із гарячим наміром («купити зараз», «доставка сьогодні»).
- У пікові моменти (День Незалежності, 14 жовтня, Новий рік) покупці бачать конкурентів з `InStock` першими = **-15-25% замовлень**.

**Рішення — розділення асортименту:**
1. Базові позиції (футболки/худі з постійних принтів, які шиються партіями на склад) → `InStock`.
2. Custom-print (індивідуальні замовлення з логотипом підрозділу) → залишити `MadeToOrder`.
3. Додати `deliveryLeadTime` у Offer schema для всіх товарів:
...
**Прогноз імпакту:** +5-10% видимість у Google Shopping по «in stock» запитам.

---

### 13.13. 🆕 LOW — CrUX field data coverage = 0% — сайт без реальних performance-метрик (підтверджено: Vilni v2 + PageSpeed Insights API)

**Джерело:** PSI API → `loadingExperience: { metrics: {} }` (порожній), field_data_coverage = 0.0%

Google Chrome User Experience Report не має статистики по twocomms.shop — Chrome-користувачі не заходять у достатній кількості для агрегації. Це означає:

1. **Google не використовує CWV у ranking-сигналі** для цього сайту — позитивний момент (бо LCP жовтий), але тимчасовий.
2. **Лабораторні цифри** LCP/INP/CLS можуть не відповідати реальному досвіду на повільних 3G/4G пристроях.

**Fix — підключити Real User Monitoring (RUM):**
- **Cloudflare Web Analytics** (безкоштовно, без cookies, privacy-friendly).
- Або **self-hosted Plausible** (якщо потрібен повний контроль).
- Через 3-4 місяці з достатнім трафіком GSC покаже свої CWV-цифри у Performance report.

> [!NOTE]
> **GSC Recovery-математика (з Vilni v2 GSC-даних):** Запит `бентежне` — 133 покази на pos 3.2 з CTR 0%. Бенчмарк CTR на позиції 3 = 9.4% (Google). Recoverable clicks = **+13/28д** = найшвидший single-query win. Фікс: переписати title для `/product/bentejne-hd/`, `/product/bentejne-ts/`, `/product/bentejne-ls/` → «Принт «Бентежне» — футболка / худі / лонгслів | TwoComms». Оцінка — 30 хвилин роботи.

---

> *Звіт підготовлений Captain Capy на основі парсингу 14 живих URL, 13 JSON-LD блоків × 14 сторінок, репозиторію twocomms.shop (24 SEO-комітів за 2026-05-15..17), даних Google Search Console (3 міс), ручної верифікації кожного твердження аудиту Vilni AI v1.0.0 та додаткового аналізу стратегії варіацій товарів від 2026-05-20.*
>
