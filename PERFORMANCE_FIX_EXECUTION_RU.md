...
- Исправлен дублирующий namespace social (теперь `social_fallback`), предупреждения compress сняты; 404 sw.js остаётся от старого файла темы, новый дубликат удалён.
- Добавлена лёгкая команда `enqueue_optimize_images` — ставит Celery-задачи на оптимизацию без нагрузки на процесс (используйте на ограниченном хостинге вместо тяжелого `optimize_images`).
...
2) Деплой на сервер: после пуша в git выполнить предоставленную команду `sshpass -p '***REMOVED_SSH_PASSWORD***' ssh -o StrictHostKeyChecking=no qlknpodo@195.191.24.169 "bash -lc 'source /home/qlknpodo/virtualenv/TWC/TwoComms_Site/twocomms/3.14/bin/activate && cd /home/qlknpodo/TWC/TwoComms_Site/twocomms && git pull'"`.  
