...
- **Transition left/top (#21):**
  - вместо `transition: left/top` на 24+ псевдо‑элементах (points-card, меню, catalog links, CTA, admin cards и т.д.) добавлен GPU‑friendly `transform: translate3d(...)`;
  - shimmer‑эффекты теперь используют CSS переменную `--shimmer-slide-duration` (0.5s/0.6s/0.8s) и больше не вызывают layout reflow на каждом наведении.
- **Template tags #17/#18:**
  - `image_optimization.py` больше не дергает `os.path.exists()` десятки раз за рендер — добавлен кэш проверок (4K элементов) и поддержка путей из `STATICFILES_DIRS`/`STATIC_ROOT`;
  - `cache_headers.py` теперь кэширует `os.path.getmtime()`, поэтому WhiteNoise не бомбит файловую систему при каждом ответе (особенно заметно на медленных дисках).
...
- Добавить инструмент для принудительного сброса anon-cache (management command) и метрики hit/miss для `cache_page_for_anon`.
