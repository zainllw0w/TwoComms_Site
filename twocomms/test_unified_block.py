#!/usr/bin/env python3
"""
Тест единого блока <pre> для Telegram уведомлений
"""
import os
import django
...
print("=== Тест единого блока <pre> для Telegram уведомлений ===")
...
try:
    ...
    for i in range(3):
        product, created = Product.objects.get_or_create(
            slug=f'test-unified-product-{i+1}',
            defaults={
                'title': f'Единый блок тестовая футболка {i+1}',
                'description': f'Единый блок тестовая футболка {i+1} для проверки единого блока <pre>',
                'price': 299 + i * 100,
                'category': category
            }
        )
    ...
    order = Order.objects.create(
        user=user,
        full_name="Единый Блок Тестовый Пользователь",
        phone="+380123456789",
        city="Киев",
    ...
    print(f"\n📱 Отправка Telegram уведомления с единым блоком <pre>...")
    notifier = TelegramNotifier()
    success = notifier.send_new_order_notification(order)
    print(f"Результат: {'✅ Успешно' if success else '❌ Ошибка'}")

    print(f"\n🎉 Тест завершен! Проверьте Telegram - должно прийти уведомление с единым блоком!")
    print(f"📋 Особенности единого блока:")
    print(f"   • Вся информация в одном блоке <pre language=\"text\">")
    print(f"   • Выровненные полоски и рамки")
    print(f"   • Красивое ASCII-арт оформление")
    print(f"   • Все данные о клиенте, заказе и товарах")
    print(f"   • Итоговая информация внизу")
