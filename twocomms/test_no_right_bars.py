#!/usr/bin/env python3
"""
Тест убранных полосок справа в Telegram уведомлениях
"""
import os
import django
...
print("=== Тест убранных полосок справа в Telegram уведомлениях ===")
...
try:
    ...
    for i in range(3):
        product, created = Product.objects.get_or_create(
            slug=f'test-nobars-product-{i+1}',
            defaults={
                'title': f'Без полосок товар {i+1}',
                'description': f'Без полосок товар {i+1} для проверки убранных полосок справа',
                'price': 299 + i * 100,
                'category': category
            }
        )
    ...
    order = Order.objects.create(
        user=user,
        full_name="Без Полосок Тест Пользователь",
        phone="+380123456789",
        city="Киев",
    ...
    print(f"\n📱 Отправка Telegram уведомления без полосок справа...")
    notifier = TelegramNotifier()
    success = notifier.send_new_order_notification(order)
    print(f"Результат: {'✅ Успешно' if success else '❌ Ошибка'}")

    print(f"\n🎉 Тест завершен! Проверьте Telegram - должно прийти уведомление без полосок справа!")
    print(f"📋 Особенности убранных полосок справа:")
    print(f"   • Убраны полоски справа из заголовков секций")
    print(f"   • Чистое отображение без лишних символов")
    print(f"   • Улучшенная читаемость")
    print(f"   • Профессиональный вид")
