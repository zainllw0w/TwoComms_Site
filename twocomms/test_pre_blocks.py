#!/usr/bin/env python3
"""
Тест форматирования Telegram уведомлений с блоками <pre>
"""
import os
import django
...
print("=== Тест форматирования с блоками <pre> ===")
...
try:
    ...
    for i in range(3):
        product, created = Product.objects.get_or_create(
            slug=f'test-pre-product-{i+1}',
            defaults={
                'title': f'PRE Тестовая футболка {i+1}',
                'description': f'PRE тестовая футболка {i+1} для проверки блоков <pre>',
                'price': 299 + i * 100,
                'category': category
            }
        )
    ...
    order = Order.objects.create(
        user=user,
        full_name="PRE Тестовый Пользователь",
        phone="+380123456789",
        city="Киев",
    ...
    print(f"\n📱 Отправка Telegram уведомления с блоками <pre>...")
    notifier = TelegramNotifier()
    success = notifier.send_new_order_notification(order)
    print(f"Результат: {'✅ Успешно' if success else '❌ Ошибка'}")

    print(f"\n🎉 Тест завершен! Проверьте Telegram - должно прийти уведомление с блоками <pre>!")
    print(f"📋 Особенности нового форматирования:")
    print(f"   • Информация о клиенте в <pre language=\"yaml\">")
    print(f"   • Детали заказа в <pre language=\"json\">")
    print(f"   • Товары в <pre language=\"text\">")
    print(f"   • Итоговая информация в <pre language=\"text\">")
    print(f"   • Красивый ASCII-арт в <pre language=\"text\">")
