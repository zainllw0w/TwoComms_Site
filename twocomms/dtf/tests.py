...
class DtfSubdomainIsolationTests(TestCase):
    ...
    def test_dtf_home_supports_en_language_switch(self):
        ...
        self.assertTrue(response.cookies["dtf_lang"]["secure"])
        self.assertTrue(response.cookies["dtf_lang"]["httponly"])
        self.assertEqual(response.cookies["dtf_lang"]["samesite"], "Lax")
        html = response.content.decode("utf-8", "ignore")
