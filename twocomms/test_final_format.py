#!/usr/bin/env python3
"""
Тест финального форматирования Telegram уведомлений
"""
import os
import django
...
print("=== Тест финального форматирования Telegram уведомлений ===")
...
try:
    ...
    for i in range(3):
        product, created = Product.objects.get_or_create(
            slug=f'test-final-product-{i+1}',
            defaults={
                'title': f'Финальная тестовая футболка {i+1}',
                'description': f'Финальная тестовая футболка {i+1} для проверки обновленного форматирования',
                'price': 299 + i * 100,
                'category': category
            }
        )
    ...
    order = Order.objects.create(
        user=user,
        full_name="Финальный Тестовый Пользователь",
        phone="+380123456789",
        city="Киев",
    ...
    print(f"\n📱 Отправка финального Telegram уведомления...")
    notifier = TelegramNotifier()
    success = notifier.send_new_order_notification(order)
    print(f"Результат: {'✅ Успешно' if success else '❌ Ошибка'}")

    print(f"\n🎉 Тест завершен! Проверьте Telegram - должно прийти финальное уведомление!")
    print(f"📋 Особенности финального форматирования:")
    print(f"   • Короткие разделители (в 2 раза меньше)")
    print(f"   • Все данные о товарах (название, размер, цвет, количество, цена)")
    print(f"   • Блок <pre language=\"text\"> с красивым ASCII-арт")
    print(f"   • Компактное и читаемое отображение")
