#!/usr/bin/env python3
"""
Тест чистого форматирования Telegram уведомлений
"""
import os
import django
...
print("=== Тест чистого форматирования Telegram уведомлений ===")
...
try:
    ...
    for i in range(3):
        product, created = Product.objects.get_or_create(
            slug=f'test-clean-product-{i+1}',
            defaults={
                'title': f'Чистый тест товар {i+1}',
                'description': f'Чистый тест товар {i+1} для проверки чистого форматирования',
                'price': 299 + i * 100,
                'category': category
            }
        )
    ...
    order = Order.objects.create(
        user=user,
        full_name="Чистый Тест Пользователь",
        phone="+380123456789",
        city="Киев",
    ...
    print(f"\n📱 Отправка Telegram уведомления с чистым форматированием...")
    notifier = TelegramNotifier()
    success = notifier.send_new_order_notification(order)
    print(f"Результат: {'✅ Успешно' if success else '❌ Ошибка'}")

    print(f"\n🎉 Тест завершен! Проверьте Telegram - должно прийти уведомление с чистым форматированием!")
    print(f"📋 Особенности чистого форматирования:")
    print(f"   • Убраны все полоски справа")
    print(f"   • Размер и цвет товаров отображаются корректно")
    print(f"   • Чистое и читаемое отображение")
    print(f"   • Все данные о товарах видны")
