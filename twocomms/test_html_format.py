#!/usr/bin/env python3
"""
Тест HTML форматирования Telegram уведомлений
"""
import os
import django
...
print("=== Тест HTML форматирования Telegram уведомлений ===")
...
try:
    ...
    for i in range(3):
        product, created = Product.objects.get_or_create(
            slug=f'test-html-product-{i+1}',
            defaults={
                'title': f'HTML Тестовая футболка {i+1}',
                'description': f'HTML тестовая футболка {i+1} для проверки красивого HTML форматирования',
                'price': 299 + i * 100,
                'category': category
            }
        )
    ...
    order = Order.objects.create(
        user=user,
        full_name="HTML Тестовый Пользователь",
        phone="+380123456789",
        city="Киев",
    ...
    print(f"\n📱 Отправка HTML Telegram уведомления...")
    notifier = TelegramNotifier()
    success = notifier.send_new_order_notification(order)
    print(f"Результат: {'✅ Успешно' if success else '❌ Ошибка'}")

    print(f"\n🎉 Тест завершен! Проверьте Telegram - должно прийти красиво отформатированное HTML уведомление!")
    print(f"📋 Особенности нового форматирования:")
    print(f"   • Градиентные заголовки")
    print(f"   • Сворачиваемый блок с товарами")
    print(f"   • Цветные теги для размеров и цветов")
    print(f"   • Красивый блок с итоговой суммой")
    print(f"   • ASCII-арт в блоке <pre>")
