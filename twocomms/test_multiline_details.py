#!/usr/bin/env python3
"""
Тест многострочного отображения деталей товаров в Telegram уведомлениях
"""
import os
import django
...
print("=== Тест многострочного отображения деталей товаров ===")
...
try:
    ...
    for i in range(3):
        product, created = Product.objects.get_or_create(
            slug=f'test-multiline-product-{i+1}',
            defaults={
                'title': f'Многострочный товар {i+1}',
                'description': f'Многострочный товар {i+1} для проверки многострочного отображения',
                'price': 299 + i * 100,
                'category': category
            }
        )
    ...
    order = Order.objects.create(
        user=user,
        full_name="Многострочный Тест Пользователь",
        phone="+380123456789",
        city="Киев",
    ...
    print(f"\n📱 Отправка Telegram уведомления с многострочными деталями...")
    notifier = TelegramNotifier()
    success = notifier.send_new_order_notification(order)
    print(f"Результат: {'✅ Успешно' if success else '❌ Ошибка'}")

    print(f"\n🎉 Тест завершен! Проверьте Telegram - должно прийти уведомление с многострочными деталями!")
    print(f"📋 Особенности многострочного отображения:")
    print(f"   • Название товара сверху")
    print(f"   • Каждая деталь на новой строке с └")
    print(f"   • Размер, количество, цвет, цена отдельно")
    print(f"   • Убраны все полоски справа")
    print(f"   • Четкое разделение между товарами")
