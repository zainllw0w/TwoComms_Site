...
class FinanceMobileDrawerStructureTests(SimpleTestCase):
    ...
    def test_finance_js_uses_pointer_drags_for_safe_smooth_drawers(self):
        ...
        self.assertRegex(js, r"EDGE_OPEN_ZONE\s*=\s*132")
        self.assertRegex(js, r"EDGE_DRAG_THRESHOLD\s*=\s*8")
        self.assertRegex(js, r"CENTER_DRAG_THRESHOLD\s*=\s*22")
        self.assertRegex(js, r"EDGE_AXIS_RATIO\s*=\s*1\.05")
        self.assertRegex(js, r"CENTER_AXIS_RATIO\s*=\s*1\.15")
        self.assertRegex(js, r"DRAWER_OPEN_RATIO\s*=\s*0\.28")
        self.assertRegex(js, r"DRAWER_MIN_RELEASE\s*=\s*44")
        self.assertIn("DRAWER_CLOSE_RATIO", js)
        self.assertIn("DRAWER_MIN_CLOSE", js)
        self.assertIn("startClosingDrawerDrag", js)
