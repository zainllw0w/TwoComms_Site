...
class CommercialEpisodeMigrationContractTests(TestCase):
    ...
    def test_backfill_treats_superseded_pointer_as_component_identity_edge(self):
        from django.apps import apps
        from management.ig_bot_models import (
            IgCommercialEpisode,
            IgPaymentConfirmationReview,
        ...
        client = IgClient.get_or_create_for_sender("episode-migration-superseded-edge")
        canonical = IgPaymentConfirmationReview.objects.create(
            client=client,
            status=IgPaymentConfirmationReview.Status.CONFIRMED,
            dedupe_key="migration-superseded-edge-canonical",
        )
        duplicate = IgPaymentConfirmationReview.objects.create(
            client=client,
            status=IgPaymentConfirmationReview.Status.SUPERSEDED,
            superseded_by=canonical,
            dedupe_key="migration-superseded-edge-duplicate",
        )
        migration = import_module("management.migrations.0106_ig_commercial_episodes")

        remaining = migration.backfill_until_quiescent(apps, None, max_passes=3)

        episode = IgCommercialEpisode.objects.get(client=client)
        client.refresh_from_db()
        self.assertEqual(episode.primary_payment_review_id, canonical.pk)
        self.assertFalse(
            IgCommercialEpisode.objects.filter(primary_payment_review=duplicate).exists()
        )
        self.assertEqual(client.current_commercial_episode_id, episode.pk)
        self.assertEqual(remaining, {"deals": 0, "reviews": 0, "attributions": 0})
    ...
    def test_backfill_validates_existing_relation_collisions_before_any_new_write(self):
        ...
        with self.assertRaisesMessage(RuntimeError, "already spans multiple"):
            migration.backfill_commercial_episodes(apps, None)
