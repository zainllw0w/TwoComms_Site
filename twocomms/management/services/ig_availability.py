...
def _resolve_warehouse(spec: AllocationSpec, *, lock: bool) -> AvailabilityDecision:
    ...
    if option_key:
        ...
        # A missing fit is not permission to pick an arbitrary warehouse
        # mapping. Keep the decision unknown until the customer supplies the
        # exact fit, unless the catalog has exactly one unambiguous mapping.
        matches = list(links.order_by("pk")[:2])
        if not matches:
            return AvailabilityDecision.unknown("inventory_mapping_missing")
        if len(matches) > 1:
            return AvailabilityDecision.unknown("inventory_mapping_ambiguous")
        link = matches[0]
