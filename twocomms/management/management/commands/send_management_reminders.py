...
from management.services.analytics_v7 import record_followup_notified
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for user in users:
            ...
            if followup_ids:
                for followup in ClientFollowUp.objects.filter(id__in=followup_ids):
                    followup.last_notified_at = now
                    followup.save(update_fields=["last_notified_at"])
                    record_followup_notified(followup=followup, notified_at=now)
