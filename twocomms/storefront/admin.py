...
class CatalogColorSeoOverrideAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Контент', {
            ...
            'description': (
                'Для «general» заповніть щонайменше одне поле: порожній рядок '
                'нічого не публікує. Для «brand»/«category» порожнє поле '
                'залишає курований варіант з коду; заповнене змінює лише цю '
                'частину блоку.'
            ),
        }),
    )
