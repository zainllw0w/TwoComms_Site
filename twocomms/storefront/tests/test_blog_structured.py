...
class BlogStructuredPublicTests(TestCase):
    ...
    def test_publish_custom_print_blog_command_creates_indexable_localized_post_landings(self):
        ...
        with tempfile.TemporaryDirectory() as media_root, override_settings(MEDIA_ROOT=media_root):
            ...
            article_schema = json.loads(response.context["article_schema"])
            self.assertEqual({item["@type"] for item in article_schema["mentions"]}, {"CollectionPage"})
            self.assertEqual(article_schema["publisher"]["@id"], "https://twocomms.shop/#organization")
            self.assertEqual(article_schema["image"], [f"https://testserver{first_post.cover_image.url}"])
            ...
            en_response = self.client.get(f"/en/blog/{first_post.slug}/", secure=True)
            self.assertEqual(en_response.status_code, 200)
            en_article_schema = json.loads(en_response.context["article_schema"])
            self.assertEqual(en_article_schema["about"][0]["name"], "Custom printing on clothing")
            self.assertEqual(en_article_schema["mentions"][0]["name"], "T-shirts for custom printing")
            self.assertEqual(en_article_schema["potentialAction"]["name"], "Create a custom print")
