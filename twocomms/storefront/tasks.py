...
def generate_google_merchant_feed_task():
    """
    Celery task to generate marketplace feed snapshots.

    Public feed endpoints are dynamic and read from the database on request.
    These snapshots are kept for backwards compatibility with integrations that
    still fetch files from MEDIA_ROOT.
    """
    try:
        logger.info("Starting marketplace feed generation task...")
        ...
        outputs = {
            ...
            'generate_buyme_feed': os.path.join(media_root, 'buyme-feed.xml'),
            'generate_prom_feed': os.path.join(media_root, 'prom-feed.xml'),
        }

        for command_name, output_path in outputs.items():
            call_command(command_name, output=output_path, verbosity=0)
            logger.info("Marketplace feed generated by %s at: %s", command_name, output_path)
