...
├─────────────────────────────────────────┤
│  👤 КЛИЕНТ:                             │
│     Имя: {order.full_name:<30} │
│     Телефон: {order.phone:<28} │
│     Город: {order.city:<30} │
│     НП: {order.np_office:<32} │
├─────────────────────────────────────────┤
│  📋 ДЕТАЛИ ЗАКАЗА:                      │
│     Статус оплаты: {order.get_payment_status_display():<20} │
│     Статус заказа: {order.get_status_display():<21} │
│     Время создания: {order.created.strftime('%d.%m.%Y %H:%M'):<18} │
├─────────────────────────────────────────┤
...
"""
        ...
        for i, item in enumerate(order.items.all(), 1):
            ...
            full_block += f"│     {item_title:<35} │\n"
            full_block += f"│        └ {item.qty} × {item.unit_price} = {item.line_total} грн{'':<15} │\n"
        ...
        full_block += f"│     Всего товаров: {total_items} шт.{'':<20} │\n"
        full_block += f"│     Сумма товаров: {subtotal} грн{'':<18} │\n"
        
        if order.promo_code:
            full_block += f"│     Промокод: {order.promo_code.code:<25} │\n"
            full_block += f"│     Скидка: -{order.discount_amount} грн{'':<20} │\n"
        
        full_block += f"├─────────────────────────────────────────┤\n"
        full_block += f"│  💳 ИТОГО К ОПЛАТЕ: {order.total_sum} грн{'':<15} │\n"
        full_block += f"└─────────────────────────────────────────┘"
        full_block += "</pre>"
