...
- admin economics widgets,
- safe `Earned Day` ledger UX.
...
### 5.5 Admin economics panel
Отдельный admin-only блок:
- attributed revenue,
- direct commission cost,
- fixed manager cost,
- contribution proxy,
- payback day,
- break-even status,
...
Это не manager-facing shaming block.
Это управленческий экран для решения: coaching, lead mix, retention, escalation.
...
- recovered days,
- excused days,
- что нужно сделать сегодня, чтобы день считался qualified.

Не показывать:
- cost-to-company,
- жёсткую punitive copy,
- формулировки “ты потерял зарплату”.
...
- excuse overrides,
- abuse flags,
- link between missed days and callback/report issues.
