...
### 6.3 Rescue SPIFF
Rescue economics фиксируются отдельно от repeat commission, чтобы удержание ценных клиентов не сводилось к плоскому бонусу.

Нормативная логика:
- `SPIFF` выплачивается только за реально rescued order из `Top-5 / rescue queue`, а не за произвольную manual claim;
- размер бонуса scaled, а не плоский:

```python
def compute_rescue_spiff(order_amount: float) -> int:
    spiff = max(500, int(order_amount * 0.01))
    return min(2000, spiff)
```

- floor = `500 грн`;
- rate = `1%` от rescued order;
- cap = `2000 грн`;
- факт rescued order должен быть подтверждён verified order truth и rescue attribution trail.
...
- expected next order;
- `P(churn_weibull)` для клиентов с достаточной историей;
...
Для клиентов с `<5` историческими заказами используется logistic fallback, а не fake individualized Weibull.
...
### 8.4 Rescue queue guardrails
Rescue queue не должна превращаться в отдельный overload channel.

Правила:
- `Top-5` сортируется по `Expected LTV Loss = avg_order_value × expected_orders_per_year × P(churn_weibull)`;
- manager получает не более `3` rescue-leads/day из pool без явного admin override;
