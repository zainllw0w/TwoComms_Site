...
from fable5.models import ProductFitNote, VariantBlankLink, VariantFitRule
    ...
    def handle(self, *args, **options):
        ...
        for product in products:
            for variant in getattr(product, "backfill_variants", []):
                ...
                existing_keys = {
                    row.option_key
                    for row in getattr(variant, "fable5_blank_links", []).all()
                }
                profile = getattr(variant.color, "fable5_profile", None)
