#!/usr/bin/env python3
"""
Тест красивого форматирования Telegram уведомлений
"""
import os
import django
...
print("=== Тест красивого форматирования Telegram уведомлений ===")
...
try:
    ...
    for i in range(3):
        product, created = Product.objects.get_or_create(
            slug=f'test-beautiful-product-{i+1}',
            defaults={
                'title': f'Тестовая футболка {i+1}',
                'description': f'Тестовая футболка {i+1} для проверки красивого форматирования',
                'price': 299 + i * 100,
                'category': category
            }
        )
    ...
    order = Order.objects.create(
        user=user,
        full_name="Тестовый Пользователь Красивый",
        phone="+380123456789",
        city="Киев",
    ...
    print(f"\n📱 Отправка Telegram уведомления с красивым форматированием...")
    notifier = TelegramNotifier()
    success = notifier.send_new_order_notification(order)
    print(f"Результат: {'✅ Успешно' if success else '❌ Ошибка'}")

    print(f"\n🎉 Тест завершен! Проверьте Telegram - должно прийти красиво отформатированное уведомление!")
