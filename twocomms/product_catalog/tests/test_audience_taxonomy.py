...
from product_catalog.services_audience import (
...
class AudienceTaxonomyTests(TestCase):
    ...
    def test_unisex_is_the_only_persisted_tag_when_gender_tags_are_also_submitted(self):
        set_product_audience_codes(self.product, ["unisex", "women"])

        self.assertEqual(get_product_audience_codes(self.product), ["unisex"])
    ...
    def test_editor_save_persists_multiple_audience_codes(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["product"]["audience_codes"], ["unisex"])
        self.assertEqual(get_product_audience_codes(self.product), ["unisex"])
