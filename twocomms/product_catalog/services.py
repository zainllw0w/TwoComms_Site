"""
Product Catalog — сервісні хелпери для ПУБЛІЧНОЇ частини сайту та генераторів фідів.

Ці функції безпечні: якщо для варіанта/кольору немає ProductCatalog-даних —
...
# Public compatibility exports for code that already imports product_catalog.services.
...
def _effective_fit_rules(variant) -> dict:
    ...
    product_notes = {
        row.fit_code: row
        for row in product.product_catalog_fit_notes.all()
    }
    color_rules = {
        row.fit_code: row
        for row in variant.product_catalog_fit_rules.all()
    }
    options = sorted(
...
def _garment_flow_for_product(product):
    ...
    cached_flows = prefetched.get("product_catalog_flows")
...
def _product_option_profiles(product) -> dict:
    ...
    rows = prefetched if prefetched is not None else product.product_catalog_option_profiles.all()
...
def _variant_combination_profiles(variant) -> dict:
    ...
    rows = prefetched if prefetched is not None else variant.product_catalog_combinations.all()
...
def _variant_details(variant):
    ...
    if "product_catalog_details" in fields_cache:
        return fields_cache.get("product_catalog_details")
...
def variant_public_context(
    ...
    *,
    ...
    if "product_catalog_profile" in color_cache:
        profile = color_cache.get("product_catalog_profile")
    else:
    ...
    return {
        ...
        "faqs": [
            ...
            for f in variant.product_catalog_faqs.all() if f.is_active
        ],
        "fit_rules": fit_rules,
        "available_fit_codes": [
...
def effective_cart_unit_price(
    ...
    if color_variant is None:
        ...
        # Products without colour rows can still expose paid ProductCatalog axes
...
def feed_includes_product(feed_slug: str, product) -> bool:
    ...
    if feed is None:
        return True  # фід не керується ProductCatalog — поводимося як раніше
